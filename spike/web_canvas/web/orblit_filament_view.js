// The JavaScript half of the Orblit web spike.
//
...
(() => {
  ...
  // The one Orblit entity this spike knows. Ids are Orblit's, not Filament's;
  ...
  class CanvasRenderer {
    ...
    apply(bytes) {
      ...
      while (at + 12 <= data.byteLength) {
        ...
        if (!target) {
          console.warn(`orblitWeb: no entity ${id}`);
          ...
          console.warn(`orblitWeb: skipped opcode ${op} with ${count} floats`);
        }
        at += 4 * count;
      }
      this.messages++;
  ...
  async function mount(canvas) {
    ...
    console.log(`orblitWeb: mounted ${canvas.id}: ${JSON.stringify(renderer.stats())}`);
    return renderer;
  }

  window.orblitWeb = {mount};
})();
