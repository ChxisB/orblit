...
void main() {
  group('atlasModel', () {
    test(
      'packs two materials onto one page and moves their UVs into it',
      () async {
        ...
        expect(
          document.json['extensionsRequired'],
          contains('KHR_texture_basisu'),
        );
        final image =
            document.list('images')[((texture['extensions']
                        as Map)['KHR_texture_basisu']
                    as Map)['source']
                as int];
    ...
    test(
      'gives a material the maps it never had, blank, so it can share',
      () async {
        final model = QuadModel();
        ...
        final normals = result.pages
            .firstWhere((p) => p.role == TextureRole.normal)
            .image;
    ...
    test(
      'folds a factor into the page so two tints become one material',
      () async {
        final model = QuadModel();
        ...
        int at(int vertex) {
          final x = (uv[vertex * 2] * page.width).floor().clamp(
            0,
            page.width - 1,
          );
          final y = (uv[vertex * 2 + 1] * page.height).floor().clamp(
            0,
            page.height - 1,
          );
    ...
    test('leaves a model whose maps tile across the surface alone', () async {
      ...
      final result = await atlasModel(document, decode: decoderOver(document));
    ...
    test(
      'leaves a material carrying an extension it does not understand',
      () async {
        final model = QuadModel();
        final a = model.texture(red);
        final b = model.texture(blue);
    ...
    test(
      'carries the parts of the document it never looked at through',
      () async {
        final model = QuadModel();
        final a = model.texture(red);
        final b = model.texture(blue);
        ...
        ((model.json['nodes'] as List).first
            as Map<String, Object?>)['extras'] = {
          'collider': 'box',
        };
