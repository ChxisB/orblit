...
`orbis_host_sdl.c` is the only file that knows what machine it is on. A port
to a console is that one file written again against an SDK that cannot be
discussed in public; `orbis_host.c`, `orbis_pad.c` and the whole renderer go
across untouched. That is the claim the directory exists to make, and the
reason the seam is drawn where `OrbisSurface.h` draws the renderer's own.
...
```sh
# macOS, Metal
packages/orbis_filament/darwin/setup.sh
packages/orbis_filament/native/host/build.sh run
```
...
routed through `orbis_pad_raw` like everything else so it exercises the same
shaping, slots and edges rather than a second path that could rot.
`ORBIS_HOST_VIRTUAL_PAD=1` attaches one of SDL's virtual gamepads and walks
...
docker run --rm -v "$PWD:/work" -v "<project>/.cache:/cache:ro" \
  orbis-linux:trixie bash -c '
    apt-get update && apt-get install -y libsdl3-dev
    cd /work/packages/orbis_filament/native/host
    ORBIS_FILAMENT_SDK=/cache/filament-1.76.0/arm-linux/filament ./build.sh
    Xvfb :99 -screen 0 1280x800x24 & sleep 2
    DISPLAY=:99 ./build/orbis_host'
```

with the materials compiled for what it will run first —
`ORBIS_MATC_BACKENDS="vulkan opengl" darwin/setup.sh`.
...
- **It is not on the link line of anything a console ships.** SDL answers
  `orbis_host_platform.h`'s eight calls on a desktop. A console answers the
...
- **SDL3 names buttons by position.** `SDL_GAMEPAD_BUTTON_SOUTH`, not
  `_A`. That is `package:orbis_input`'s decision, arrived at independently,
  which is why `orbis_host_sdl.c`'s mapping table is sixteen entries and has
...
amount of code as `orbis_host_sdl.c` **per platform**, and three of them would
...
`-DFILAMENT_SUPPORTS_WEBGPU=ON` draws Orbis's scene through Dawn onto Metal,
