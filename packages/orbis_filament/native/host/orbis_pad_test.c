/* That orbis_pad.c still agrees with package:orbis_input.
 *
 * orbis_pad.h says the two drifting apart is a bug in whichever moved, which
 * is a claim nobody can check by reading. So the cases below are
 * packages/orbis_input/test/shaping_test.dart and pads_test.dart, the same
...
static orbis_pad_raw raw_pad(const char *token, uint32_t buttons, float lx,
                             float ly) {
  orbis_pad_raw pad;
  ...
  pad.name = token;
  pad.buttons = buttons;
  pad.stick_x[ORBIS_PAD_LEFT] = lx;
  pad.stick_y[ORBIS_PAD_LEFT] = ly;
  return pad;
}

static float stick_length(const orbis_shaping *shaping, float x, float y) {
  float sx = 0;
  float sy = 0;
  orbis_shape_stick(shaping, x, y, &sx, &sy);
...
int main(void) {
  ...
  group = "defaults";
  near(orbis_shaping_sticks.inner, 0.15f, "a stick's dead zone");
  near(orbis_shaping_sticks.outer, 0.95f, "a stick saturates early");
  near(orbis_shaping_sticks.curve, 1.0f, "linear until a game says otherwise");
  near(orbis_shaping_triggers.inner, 0.06f, "a trigger's smaller dead zone");
  near(orbis_shaping_triggers.outer, 0.98f, "a trigger saturates later");
  near(orbis_shaping_triggers.curve, 1.0f, "a throttle is linear");

  /* ---- The dead zone ---- */
  const orbis_shaping zone = {0.2f, 0.9f, 1.0f};
  ...
  float x = 0;
  float y = 0;
  orbis_shape_stick(&zone, 0.2001f, 0, &x, &y);
  ...
  orbis_shape_stick(&zone, 0.9f, 0, &x, &y);
  near(x, 1.0f, "the outside saturates before the corner");
  orbis_shape_stick(&zone, 1.0f, 0, &x, &y);
  near(x, 1.0f, "and stays there past it");

  near(orbis_shape_scalar(&zone, -0.9f), -1.0f, "a scalar keeps its sign");
  near(orbis_shape_scalar(&zone, 0.9f), 1.0f, "and the other one");

  /* ---- The zone is round, not square ---- */
  const orbis_shaping round_zone = {0.2f, 1.0f, 1.0f};
  ...
  for (int i = 0; i < 3; i++) {
    ...
    orbis_shape_stick(&round_zone, length, 2.0f * length, &x, &y);
    near(atan2f(y, x), bearing, "the same bearing, whatever the length");
  }

  /* ---- Buttons, axes and edges ---- */
  orbis_pads *pads = orbis_pads_create();
  orbis_pad_frame frame;
  check(pads != NULL, "a pad table can be made");
  if (pads == NULL) return 1;

  group = "edges";
  orbis_pad_raw one = raw_pad("uniq:a", 1u << ORBIS_PAD_SOUTH, 0, 0);
  orbis_pads_poll(pads, &one, 1, &frame);
  check(orbis_pad_is_down(&frame.pads[0], ORBIS_PAD_SOUTH), "held is down");
  check(orbis_pad_was_pressed(&frame.pads[0], ORBIS_PAD_SOUTH),
        "and pressed on the poll it arrived");
  orbis_pads_poll(pads, &one, 1, &frame);
  check(orbis_pad_is_down(&frame.pads[0], ORBIS_PAD_SOUTH),
        "still down while held");
  check(!orbis_pad_was_pressed(&frame.pads[0], ORBIS_PAD_SOUTH),
        "a press is reported for exactly one poll");
  one.buttons = 0;
  orbis_pads_poll(pads, &one, 1, &frame);
  check((frame.pads[0].released & (1u << ORBIS_PAD_SOUTH)) != 0,
  ...
  orbis_pads_poll(pads, &one, 1, &frame);
  check(frame.pads[0].stick_y[ORBIS_PAD_LEFT] > 0.0f,
        "up is positive, whatever the hardware says");
  near(frame.pads[0].raw_stick_y[ORBIS_PAD_LEFT], 0.8f,
       "and the raw value is kept beside the shaped one, flipped too");

  one = raw_pad("uniq:a", 0, 0, 0);
  one.trigger[ORBIS_PAD_RIGHT] = 0.5f;
  orbis_pads_poll(pads, &one, 1, &frame);
  check(frame.pads[0].trigger[ORBIS_PAD_RIGHT] > 0.0f,
        "a trigger runs nought to one");
  check(frame.pads[0].trigger[ORBIS_PAD_LEFT] == 0.0f, "and never negative");
  one.trigger[ORBIS_PAD_RIGHT] = 1.0f;
  orbis_pads_poll(pads, &one, 1, &frame);
  near(frame.pads[0].trigger[ORBIS_PAD_RIGHT], 1.0f,
       "fully pulled is exactly one");

  /* "A trigger past half way is also a button", measured where orbis_input
  ...
  one.trigger[ORBIS_PAD_RIGHT] = 0.5f;
  orbis_pads_poll(pads, &one, 1, &frame);
  check(!orbis_pad_is_down(&frame.pads[0], ORBIS_PAD_RIGHT_TRIGGER),
        "half of the raw travel is not yet half of the shaped travel");
  one.trigger[ORBIS_PAD_RIGHT] = 1.0f;
  orbis_pads_poll(pads, &one, 1, &frame);
  check(orbis_pad_is_down(&frame.pads[0], ORBIS_PAD_RIGHT_TRIGGER),
        "a trigger past half way is also a button");
  check(orbis_pad_was_pressed(&frame.pads[0], ORBIS_PAD_RIGHT_TRIGGER),
        "and crossing the line is a press, like any other button");
  one.trigger[ORBIS_PAD_RIGHT] = 0.0f;
  orbis_pads_poll(pads, &one, 1, &frame);
  check(!orbis_pad_is_down(&frame.pads[0], ORBIS_PAD_RIGHT_TRIGGER),
        "and letting go is a release");
  check(!orbis_pad_is_down(&frame.pads[0], ORBIS_PAD_LEFT_TRIGGER),
        "the other trigger was never touched");

  group = "axes";

  group = "d-pad";
  one = raw_pad("uniq:a", (1u << ORBIS_PAD_DPAD_UP) | (1u << ORBIS_PAD_DPAD_RIGHT),
                0, 0);
  orbis_pads_poll(pads, &one, 1, &frame);
  orbis_pad_dpad(&frame.pads[0], &x, &y);
  near(x, 1.0f, "the d-pad is a vector, right");
  near(y, 1.0f, "the d-pad is a vector, up");

  /* ---- Slots, absence and hot-plug ---- */
  group = "slots";
  orbis_pads_destroy(pads);
  pads = orbis_pads_create();
  if (pads == NULL) return 1;

  orbis_pad_raw two[2] = {raw_pad("uniq:a", 0, 0, 0), raw_pad("uniq:b", 0, 0, 0)};
  orbis_pads_poll(pads, two, 2, &frame);
  check(orbis_pad_count(&frame) == 2, "two pads take two slots");
  ...
  two[0] = raw_pad("uniq:a", 1u << ORBIS_PAD_DPAD_LEFT, -1.0f, 0);
  orbis_pads_poll(pads, two, 2, &frame);
  orbis_pad_raw only_b = raw_pad("uniq:b", 0, 0, 0);
  orbis_pads_poll(pads, &only_b, 1, &frame);
  check(!frame.pads[0].connected, "a pad that vanishes is absent");
  check(frame.pads[0].down == 0 && frame.pads[0].stick_x[ORBIS_PAD_LEFT] == 0.0f,
  ...
  orbis_pad_raw back[2] = {raw_pad("uniq:b", 0, 0, 0), raw_pad("uniq:a", 0, 0, 0)};
  orbis_pads_poll(pads, back, 2, &frame);
  ...
  pads = orbis_pads_create();
  if (pads == NULL) return 1;
  orbis_pad_raw many[ORBIS_PAD_SLOTS + 2];
  char names[ORBIS_PAD_SLOTS + 2][8];
  for (int i = 0; i < ORBIS_PAD_SLOTS + 2; i++) {
    snprintf(names[i], sizeof names[i], "p%d", i);
    many[i] = raw_pad(names[i], 0, 0, 0);
    many[i].name = names[i];
  }
  orbis_pads_poll(pads, many, ORBIS_PAD_SLOTS + 2, &frame);
  check(orbis_pad_count(&frame) == ORBIS_PAD_SLOTS,
  ...
  pads = orbis_pads_create();
  if (pads == NULL) return 1;
  orbis_pads_poll(pads, NULL, 0, &frame);
  check(orbis_pad_count(&frame) == 0, "no pads is a state, not a failure");
  const orbis_pad_state *first = orbis_pad_first(&frame);
  check(first != NULL, "and first is never NULL");
  check(!first->connected && first->down == 0, "it is a pad holding nothing");
  check(orbis_pad_is_active(first) == 0, "which nobody is touching");
  check(orbis_pad_is_down(NULL, ORBIS_PAD_SOUTH) == 0, "NULL is safe too");

  group = "first";
  orbis_pad_raw second_only = raw_pad("uniq:b", 0, 0, 0);
  orbis_pads_poll(pads, &second_only, 1, &frame);
  check(orbis_pad_first(&frame)->connected,
        "first is the lowest connected pad");

  orbis_pads_destroy(pads);

  if (failures == 0) {
    printf("orbis_pad: every case passed\n");
    return 0;
  }
  printf("orbis_pad: %d failed\n", failures);
  return 1;
}
