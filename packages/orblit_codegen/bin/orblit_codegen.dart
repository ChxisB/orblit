...
///
/// Usage: dart run orblit_codegen [package-root]
void main(List<String> arguments) {
  ...
  if (result.hasErrors) {
    stderr.writeln('orblit_codegen found ${result.errors.length} problem(s):');
  ...
  final dartOut = File(p.join(root, 'lib', 'orblit_components.g.dart'))
    ..createSync(recursive: true)
    ..writeAsStringSync(emitter.dart(className, result.components));

  final manifestOut = File(p.join(root, 'orblit_components.json'))
    ..writeAsStringSync(emitter.manifest(packageName, result.components));

  stdout.writeln('orblit_codegen: ${result.components.length} component(s)');
