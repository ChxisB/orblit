...
/// devtmpfs elsewhere and names it in ORBLIT_INPUT_DEV. See
...
import 'package:orblit_input/orblit_input.dart';
...
final String deviceDirectory =
    Platform.environment['ORBLIT_INPUT_DEV'] ?? '/dev/input';
...
void main() {
  ...
  group('a pad the kernel invented', () {
    ...
    test('is found, and is the pad we made', () async {
      ...
      expect(state.identity.name, 'Orblit Virtual Pad');
  ...
  test('a second pad gets the second slot', () async {
    final one = VirtualPad.create(name: 'Orblit Virtual Pad One');
    final pads = Pads(backend: LinuxPads(devices: deviceDirectory));
    await until(pads, (frame) => frame.count > 0);

    final two = VirtualPad.create(name: 'Orblit Virtual Pad Two');
