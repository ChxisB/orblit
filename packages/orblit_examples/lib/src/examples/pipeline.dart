...
class PipelineExample extends Example {
  PipelineExample() {
    ...
    final wanted = Platform.environment['ORBLIT_DETAIL'];
    if (wanted != null) {
      for (final one in OrblitDetail.values) {
  ...
  OrblitDetail detail = OrblitDetail.medium;
  OrblitPipeline pipeline = OrblitPipeline.at(OrblitDetail.medium);

  void _retier(OrblitDetail chosen) {
    detail = chosen;
    pipeline = OrblitPipeline.at(chosen);
  }
  ...
  OrblitScene scene(OrblitCamera camera, double seconds) {
    ...
    return OrblitScene(
      ...
      sky: OrblitSky(colour: Vector3(0.24, 0.33, 0.46), ambient: 22000),
    );
  }
