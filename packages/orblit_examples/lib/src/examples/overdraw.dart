...
/// is handed them.
///
/// That is no longer a prediction. [OrblitScene.depthPrepass] builds the second
...
///
/// ```sh
/// ORBLIT_EXAMPLE=Overdraw ORBLIT_SLABS=96 ORBLIT_SECONDS=2 ORBLIT_CIRCLING=0 \
///   ORBLIT_PREPASS=0 ORBLIT_DUMP_FRAME=180 orblit_gallery
/// ```
///
/// and again with `ORBLIT_PREPASS=1`; the frame's cost is on the
/// `[orblit] frame 180:` line. Gallery frame dumps share one path, so two of
...
/// host (`ORBLIT_SLABS`) and run under Mesa's llvmpipe in a Linux container — a
...
class OverdrawExample extends Example {
  ...
  OrblitScene scene(OrblitCamera camera, double seconds) {
    ...
    return OrblitScene(
      ...
      sky: OrblitSky(colour: linearOf(const Color(0xFF1D242E)), ambient: 9000),
      camera: camera,
    );
  }
