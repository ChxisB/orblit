...
class OutlineExample extends Example {
  ...
  OrblitOccluded occluded = OrblitOccluded.dashed;
  ...
  OrblitObject _box(int key, Vector3 at, Vector3 size, Color colour) =>
      OrblitObject(
        key: key,
  ...
  @override
  OrblitScene scene(OrblitCamera camera, double seconds) => OrblitScene(
    objects: [
      _box(
        _floor,
        Vector3(0, -1.1, 0),
    ...
    camera: camera,
    post: OrblitPostProcess(antiAliasing: antiAliasing),
    outline: on
        ? OrblitOutline(
            primary: _behind,
  ...
  Widget settings(BuildContext context, VoidCallback changed) => Column(
    ...
    children: [
      ...
      Choice(
        ...
        options: [for (final style in OrblitOccluded.values) style.name],
        selected: occluded.name,
        onSelect: (value) {
          occluded = OrblitOccluded.values.byName(value);
          changed();
        },
      ),
      Choice(
