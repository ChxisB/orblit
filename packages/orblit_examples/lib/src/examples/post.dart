...
class PostExample extends Example {
  ...
  final OrblitPostProcess post = OrblitPostProcess(
    bloom: OrblitBloom(enabled: true, strength: 0.25),
    occlusion: OrblitOcclusion(enabled: true),
    grading: OrblitGrading(enabled: true, contrast: 1.05),
  );
  ...
  OrblitScene scene(OrblitCamera camera, double seconds) {
    ...
    return OrblitScene(
      ...
      sky: OrblitSky(colour: Vector3(0.10, 0.13, 0.19), ambient: 12000),
    );
  }
...
OrblitScene(
  ...
  post: OrblitPostProcess(
    bloom: OrblitBloom(enabled: true, strength: 0.25, lensFlare: true),
    occlusion: OrblitOcclusion(enabled: true),
    depthOfField: OrblitDepthOfField(enabled: true, focusDistance: 12),
