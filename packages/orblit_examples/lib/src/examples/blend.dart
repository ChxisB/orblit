...
class BlendExample extends Example {
  ...
  static const _panels = <(OrblitBlendMode, String)>[
    (OrblitBlendMode.linear, 'Linear'),
    (OrblitBlendMode.masked, 'Masked'),
    (OrblitBlendMode.maskedDepth, 'Masked depth'),
  ];
  ...
  OrblitScene scene(OrblitCamera camera, double seconds) {
    return OrblitScene(
      ...
      materials: [
        for (var i = 0; i < _panels.length; i++)
          OrblitMaterial(
            ...
            roughness: 0.78,
            baseColourMap: _cobbles == null ? null : OrblitTexture(_cobbles!),
            blendMode: _ready ? _panels[i].$1 : OrblitBlendMode.none,
            blendAmount: amount,
            blendSharpness: sharpness,
            blendBaseColourMap: _grass == null ? null : OrblitTexture(_grass!),
            blendMaskMap: _height == null
                ? null
                : OrblitTexture(_height!, srgb: false),
            tiling: Vector2(2, 2),
      ...
      pipeline: OrblitPipeline(
        shadows: OrblitShadows(kind: OrblitShadowKind.soft, distance: 40),
      ),
    );
  }
...
''';
  ...
  Future<void> _draw() async {
    final directory = Directory(
      '${Directory.systemTemp.path}/orblit_gallery_blend',
    );
