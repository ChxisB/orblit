...
class ShadowsExample extends Example {
  ...
  final OrblitShadows shadows = OrblitShadows(cascades: 3, distance: 60);
  ...
  OrblitScene scene(OrblitCamera camera, double seconds) {
    ...
    return OrblitScene(
      objects: objects,
      camera: camera,
      pipeline: OrblitPipeline(shadows: shadows),
      lights: [_light()],
      sky: OrblitSky(colour: Vector3(0.24, 0.33, 0.46), ambient: 9000),
    );
  }

  OrblitLight _light() => switch (light) {
    ShadowLight.sun => OrblitLight(
      key: 900,
  ...
  List<Widget> _shadowSettings(VoidCallback changed) => [
    Choice(
      label: 'Edge',
      options: [for (final one in OrblitShadowKind.values) one.label],
    ...
    if (shadows.kind == OrblitShadowKind.soft ||
        shadows.kind == OrblitShadowKind.area)
      Setting(
        label: 'Softness',
    ...
    if (shadows.kind == OrblitShadowKind.area) ...[
      Setting(
        label: 'Light size',
        value: lightSize,
        min: 0.02,
    ...
    if (shadows.kind == OrblitShadowKind.variance) ...[
      Setting(
        label: 'Blur',
