...
class DecalsExample extends Example {
  ...
  Future<void> _draw() async {
    final directory = Directory(
      '${Directory.systemTemp.path}/orblit_gallery_decals',
    );
  ...
  OrblitTexture? _picture(String? path) =>
      path == null ? null : OrblitTexture(path);

  /// Every decal in the scene, in the order [only] counts them.
  List<OrblitDecal> get _decals {
    final fadeStart = angleFade
        ? OrblitDecal.defaultFadeStart
        : OrblitDecal.noFade;
    final fadeEnd = angleFade ? OrblitDecal.defaultFadeEnd : OrblitDecal.noFade;
  ...
  OrblitScene scene(OrblitCamera camera, double seconds) {
    ...
    return OrblitScene(
      ...
      sky: OrblitSky(colour: linearOf(const Color(0xFF8FA6C0)), ambient: 18000),
      camera: camera,
    );
  }
