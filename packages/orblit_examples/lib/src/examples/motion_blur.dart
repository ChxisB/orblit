...
class MotionBlurExample extends Example {
  ...
  OrblitObject _box(int key, Matrix4 transform, Color colour) => OrblitObject(
    key: key,
    transform: transform,
  ...
  OrblitScene scene(OrblitCamera camera, double seconds) {
    ...
    return OrblitScene(
      ...
      graph: on ? OrblitMotionBlur(objects: objects).graph() : null,
    );
  }
...
OrblitScene(
  ...
  graph: const OrblitMotionBlur().graph(),
  // ...
)

// Or as one pass in a graph of your own, reading a target with depth.
const OrblitMotionBlur(maxPixels: 32, objects: false)
