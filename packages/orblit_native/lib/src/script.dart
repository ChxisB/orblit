...
typedef _StartFn = Void Function(Pointer<OrblitScriptHost>);
...
class NativeScript {
  ...
  final void Function(Pointer<OrblitScriptHost>) _start;
  ...
  static NativeScript open(File library) {
    ...
    final abi = find('orblit_script_abi');
    if (abi == nullptr) {
      refuse(
        '${_nameOf(library)} does not look like a script: it has no '
        'orblit_script_abi. Did it use the ORBLIT_SCRIPT macro?',
      );
    }
    ...
    return NativeScript._(
      ...
      library,
      _nameOf(library),
      entries['orblit_start']!
          .cast<NativeFunction<_StartFn>>()
          .asFunction<void Function(Pointer<OrblitScriptHost>)>(),
      entries['orblit_step']!
