...
#ifndef ORBLIT_SCRIPT_H
#define ORBLIT_SCRIPT_H
...
/// every entry point.
typedef struct OrblitScriptHost {
  /// ORBLIT_SCRIPT_ABI as the *host* was built. A script that wants to run
  ...
  /// sizeof(OrblitScriptHost) as the host was built.
  ///
  ...
  // --- the core, by pointer -------------------------------------------------
  //
  // The same functions declared in orblit_core.h. Reached this way rather than
  // by linking, so that a script is a file the host loads rather than a file
  // that has to have found the engine first.

  OrblitComponent (*component_register)(OrblitWorld *world, const char *name,
                                       uint32_t size, uint32_t alignment);
  OrblitComponent (*component_lookup)(const OrblitWorld *world, const char *name);

  OrblitEntity (*entity_create)(OrblitWorld *world);
  void (*entity_destroy)(OrblitWorld *world, OrblitEntity entity);
  bool (*entity_alive)(const OrblitWorld *world, OrblitEntity entity);
  uint32_t (*entity_count)(const OrblitWorld *world);
  bool (*entity_add)(OrblitWorld *world, OrblitEntity entity,
                     OrblitComponent component, const void *value);
  bool (*entity_remove)(OrblitWorld *world, OrblitEntity entity,
                        OrblitComponent component);
  bool (*entity_has)(const OrblitWorld *world, OrblitEntity entity,
                     OrblitComponent component);
  void *(*entity_get)(OrblitWorld *world, OrblitEntity entity,
                      OrblitComponent component);

  OrblitQuery *(*query_create)(OrblitWorld *world,
                              const OrblitComponent *components, uint32_t count);
  void (*query_destroy)(OrblitQuery *query);
  uint32_t (*query_chunk_count)(OrblitQuery *query);
  uint32_t (*query_chunk_length)(OrblitQuery *query, uint32_t chunk);
  void *(*query_chunk_column)(OrblitQuery *query, uint32_t chunk, uint32_t slot);
  const OrblitEntity *(*query_chunk_entities)(OrblitQuery *query, uint32_t chunk);

  OrblitTransforms (*transform_register)(OrblitWorld *world);
...
///
/// Defined for you by ORBLIT_SCRIPT, which is why it is the one entry point
/// nobody writes by hand: a number somebody has to remember to update is a
/// number that will be wrong.
uint32_t orblit_script_abi(void);

/// sizeof(OrblitScriptHost) as this script was built.
///
/// Also defined for you by ORBLIT_SCRIPT. Two builds can agree about the
...
/// The host, for everything below. Null before orblit_start.
inline const OrblitScriptHost *&host_slot() {
  static const OrblitScriptHost *held = nullptr;
  return held;
}

inline const OrblitScriptHost *host() { return host_slot(); }

inline OrblitWorld *world() { return host_slot()->world; }

inline void log(const char *message) { host_slot()->log(message); }

/// A component id, registered on first use.
template <typename T>
inline OrblitComponent component(const char *name) {
  return host_slot()->component_register(world(), name, sizeof(T), alignof(T));
}

inline OrblitEntity spawn() { return host_slot()->entity_create(world()); }

inline void destroy(OrblitEntity entity) {
  host_slot()->entity_destroy(world(), entity);
}

/// Adds a component with a value, in one call and with the type kept.
template <typename T>
inline bool give(OrblitEntity entity, OrblitComponent id, const T &value) {
...
/// is the same pointer.
template <typename T>
inline T *get(OrblitEntity entity, OrblitComponent id) {
...
///       static const double *at = ::orblit::number_at(asset, "speed", 0.0);
...
/// includes.
#define ORBLIT_SCRIPT                                       \
  extern "C" uint32_t orblit_script_abi(void) {             \
    return ORBLIT_SCRIPT_ABI;                               \
  }                                                        \
  extern "C" uint32_t orblit_script_host_size(void) {       \
    return (uint32_t)sizeof(OrblitScriptHost);              \
  }                                                        \
  static void orblit_script_started();                      \
  extern "C" void orblit_start(const OrblitScriptHost *h) {  \
    ::orblit::host_slot() = h;                              \
    orblit_script_started();                                \
  }                                                        \
  static void orblit_script_started()

#endif  // __cplusplus

#endif  // ORBLIT_SCRIPT_H
