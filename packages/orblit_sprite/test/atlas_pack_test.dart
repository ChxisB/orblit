...
Uint8List _withTransparentMargin(
  int w,
  int h,
  int offsetX,
  int offsetY,
...
({int x, int y, int w, int h}) _physBox(Region r) => (
  x: r.x,
  y: r.y,
  w: r.rotated ? r.height : r.width,
  h: r.rotated ? r.width : r.height,
);
...
void main() {
  group('packing rectangles', () {
    test('nothing overlaps, everything sits inside its page', () {
      ...
      for (final page in result.pages) {
        final boxes = [
          for (final region in page.regions.values) _physBox(region),
        ];
        ...
        for (var i = 0; i < boxes.length; i++) {
          for (var j = i + 1; j < boxes.length; j++) {
            expect(
              _rectsOverlap(boxes[i], boxes[j]),
              isFalse,
              reason: 'regions $i and $j overlap',
            );
          }
        }
      }
      ...
      for (final page in result.pages) {
        ...
        for (var i = 0; i < boxes.length; i++) {
          ...
          for (var j = 0; j < boxes.length; j++) {
            if (i == j) continue;
            expect(
              _rectsOverlap(grown, boxes[j]),
              isFalse,
              reason: 'boxes $i and $j are closer than padding',
            );
          }
        }
      }
      ...
      final sprites = [
        for (var i = 0; i < 200; i++)
          AtlasSprite(
            ...
            width: 8,
            height: 8,
            pixels: _solid(8, 8, 9, 9, 9, 255),
          ),
      ];
      final result = packAtlas(
        sprites,
        const AtlasPackOptions(maxPageSize: 64, trim: false),
      );
      for (final page in result.pages) {
  ...
  group('rotation', () {
    test('a rotated region reads back rotated the right way round', () {
      ...
      // a valid pack at all.
      final filler = AtlasSprite(
        name: 'filler',
        width: 6,
        height: 10,
        pixels: _solid(6, 10, 5, 5, 5, 255),
      );
      final bannerPixels = _asymmetricPattern(8, 3);
      final banner = AtlasSprite(
        name: 'banner',
        width: 8,
        height: 3,
        pixels: bannerPixels,
      ...
      expect(
        region.rotated,
        isTrue,
        reason: 'the only free space left was too narrow unless it turned',
      );
  ...
  group('trimming', () {
    ...
    test('a sprite with no transparent margin is not marked trimmed', () {
      ...
      final result = packAtlas([
        sprite,
      ], const AtlasPackOptions(maxPageSize: 32, trim: true));
  ...
  group('extrusion', () {
    test('a placed sprite bleeds its edge colour into its padding', () {
      final sprite = AtlasSprite(
        ...
        width: 4,
        height: 4,
        pixels: _solid(4, 4, 200, 100, 50, 255),
      );
      final result = packAtlas(
        [sprite],
        const AtlasPackOptions(
      ...
      final right1 = page.pixels.sublist(
        at(region.x + 4, region.y),
        at(region.x + 4, region.y) + 4,
      );
      final right2 = page.pixels.sublist(
        at(region.x + 5, region.y),
        at(region.x + 5, region.y) + 4,
      );
      expect(right1, [200, 100, 50, 255]);
      expect(right2, [200, 100, 50, 255]);

      final below1 = page.pixels.sublist(
        at(region.x, region.y + 4),
        at(region.x, region.y + 4) + 4,
      );
    ...
    test('nothing is extruded when extrude is nought', () {
      ...
      final region = page.regions['chip']!;
      int at(int x, int y) => (y * page.width + x) * 4;
      final right = page.pixels.sublist(
        at(region.x + 4, region.y),
        at(region.x + 4, region.y) + 4,
      );
  ...
  group('multiple pages', () {
    test(
      'a full page overflows into another rather than growing without limit',
      () {
        final sprites = [
          for (var i = 0; i < 25; i++)
            AtlasSprite(
        ...
        final result = packAtlas(
          sprites,
          const AtlasPackOptions(maxPageSize: 32, padding: 1, trim: false),
        );
        expect(result.problems, isEmpty);
      ...
      final sprites = [
        for (var i = 0; i < 25; i++)
          AtlasSprite(
            ...
            width: 12,
            height: 12,
            pixels: _solid(12, 12, i % 256, (i * 3) % 256, (i * 7) % 256, 255),
          ),
      ];
      final result = packAtlas(
        sprites,
  ...
  group('duplicates', () {
    ...
    test('turning duplicate merging off packs every sprite on its own', () {
      ...
      final page = result.pages.single;
      expect(
        (page.regions['a']!.x, page.regions['a']!.y) ==
            (page.regions['b']!.x, page.regions['b']!.y),
        isFalse,
      );
    });
  });
    ...
    test('pack, write and read gives back an equal atlas', () {
      final sprites = [
        AtlasSprite(
          ...
          width: 8,
          height: 3,
          pixels: _asymmetricPattern(8, 3),
        ),
        AtlasSprite(
          name: 'trimmed',
          width: 12,
          height: 10,
          pixels: _withTransparentMargin(12, 10, 2, 1, 8, 7),
        ),
        AtlasSprite(
          name: 'plain',
          width: 5,
          height: 5,
          pixels: _solid(5, 5, 1, 2, 3, 255),
        ),
      ];
      final result = packAtlas(
        sprites,
        const AtlasPackOptions(maxPageSize: 64),
      );
      for (final page in result.pages) {
        ...
        for (final name in atlas.regions.keys) {
          ...
          expect(
            actual.rotated,
            expectedRegion.rotated,
            reason: '$name.rotated',
          );
          expect(
            actual.trimmed,
            expectedRegion.trimmed,
            reason: '$name.trimmed',
          );
          expect(
            actual.offsetX,
            expectedRegion.offsetX,
            reason: '$name.offsetX',
          );
          expect(
            actual.offsetY,
            expectedRegion.offsetY,
            reason: '$name.offsetY',
          );
          expect(
            actual.sourceWidth,
            expectedRegion.sourceWidth,
            reason: '$name.sourceWidth',
          );
          expect(
            actual.sourceHeight,
            expectedRegion.sourceHeight,
            reason: '$name.sourceHeight',
          );
        }
      }
    });
  ...
  group('the background entry point', () {
    test('packing off the calling isolate gives the same answer', () async {
      ...
      expect(
        background.pages.single.pixels,
        equals(direct.pages.single.pixels),
      );
    });
  });
  ...
  group('edge cases', () {
    test(
      'a sprite larger than any page is refused with a reason, not thrown',
      () {
        final sprites = [
          AtlasSprite(
            name: 'huge',
            width: 200,
            height: 200,
            pixels: _solid(200, 200, 1, 1, 1, 255),
          ),
          AtlasSprite(
            name: 'fine',
            width: 10,
            height: 10,
            pixels: _solid(10, 10, 2, 2, 2, 255),
          ),
        ];
        final result = packAtlas(
          sprites,
    ...
    test('a zero-size sprite is handled rather than thrown', () {
      final sprites = [
        ...
        AtlasSprite(
          ...
          width: 6,
          height: 6,
          pixels: _solid(6, 6, 1, 1, 1, 255),
        ),
      ];
      final result = packAtlas(
        sprites,
        const AtlasPackOptions(maxPageSize: 32),
      );
      expect(result.problems, isEmpty);
  ...
  group('heuristics', () {
    ...
    test('left to choose, the packer picks one of the five and says which', () {
      ...
      final result = packAtlas(
        sprites,
        const AtlasPackOptions(maxPageSize: 128, trim: false),
      );
