...
OrblitPopulation crowd({required int count, int revision = 1, int key = 1}) =>
    OrblitPopulation(
      key: key,
...
OrblitScene sceneOf(List<OrblitPopulation> populations) => OrblitScene(
  objects: const [],
...
void _fadeTests() {
  group('how a member goes at the range', () {
    ...
    test('each mode is its own value in bits two and three', () {
      expect((one(fade: OrblitFade.sink).flags >> 2) & 3, 0);
      expect((one(fade: OrblitFade.shrink).flags >> 2) & 3, 1);
      expect((one(fade: OrblitFade.none).flags >> 2) & 3, 2);
    });
    ...
    test('every mode fits in the two bits it is given', () {
      ...
      // adding one.
      expect(OrblitFade.values.length, lessThanOrEqualTo(4));
    });
    ...
    test('it does not disturb the shadow bits or the layer', () {
      ...
      final planted = OrblitPopulation(
        key: 1,
        transforms: Float32List(16),
        colours: Float32List(3),
        minimum: Vector3.zero(),
