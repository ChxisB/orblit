...
void main() {
  OrblitScene sceneOf({bool? batching}) {
    final objects = [
      for (var i = 0; i < 6; i++)
        OrblitObject(
          key: 100 + i,
    ...
    return batching == null
        ? OrblitScene(objects: objects, camera: camera)
        : OrblitScene(objects: objects, camera: camera, batching: batching);
  }
  ...
  test('survives copyWith, and can be changed by it', () {
    ...
    expect(on.copyWith(sky: OrblitSky()).batching, isTrue);
    ...
    expect(off.copyWith(sky: OrblitSky()).batching, isFalse);
    expect(off.copyWith(sky: OrblitSky()).toMessage(1)['batching'], isFalse);
  });
}
