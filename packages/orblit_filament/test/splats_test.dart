...
void main() {
  OrblitScene sceneWith(List<OrblitSplats> splats) => OrblitScene(
    objects: const [],
    splats: splats,
    camera: OrblitCamera(position: Vector3(0, 0, 5), target: Vector3.zero()),
  );

  Uint8List two() => OrblitSplats.pack(
  ...
  group('packing', () {
    test('is thirty-two bytes a splat, in the .splat layout', () {
      final bytes = two();
      expect(bytes.length, 2 * OrblitSplats.recordBytes);
  ...
  group('the message', () {
    ...
    test('carries an in-memory cloud until the renderer has it', () {
      final cloud = OrblitSplats(key: 7, data: two(), revision: 3);
    ...
    test('sends a file by its path and no records', () {
      final message = sceneWith([
        OrblitSplats(key: 1, path: '/captures/garden.ply'),
      ]).toMessage(1);
    ...
    test('packs the transform, opacity and brightness at the stride', () {
      final message = sceneWith([
        OrblitSplats(key: 1, path: 'a.splat'),
        OrblitSplats(
          key: 2,
          path: 'b.splat',
      ...
      expect(params.length, 2 * OrblitSplats.stride);
      const second = OrblitSplats.stride;
  ...
  group('the flags', () {
    test('carry the sort in the low bit and the degree above it', () {
      expect(OrblitSplats(key: 1, path: 'a.ply').flags, 1 | (2 << 1));
      expect(OrblitSplats(key: 1, path: 'a.ply', harmonics: 0).flags, 1);
      expect(
        OrblitSplats(key: 1, path: 'a.ply', harmonics: 1).flags,
        1 | (1 << 1),
      );
      expect(
        OrblitSplats(key: 1, path: 'a.ply', sorted: false, harmonics: 3).flags,
        3 << 1,
      );
    });
    ...
    test('refuse a degree no capture is trained to', () {
      ...
      expect(
        () => OrblitSplats(key: 1, path: 'a.ply', harmonics: -1),
        throwsA(isA<AssertionError>()),
      );
    });
  });
  ...
  test('a cloud is a file or a set of records, never both', () {
    expect(
      () => OrblitSplats(key: 1, path: 'a.ply', data: Uint8List(32)),
      throwsA(isA<AssertionError>()),
    );
    expect(() => OrblitSplats(key: 1), throwsA(isA<AssertionError>()));
  });
}
