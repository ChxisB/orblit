...
void main() {
  group('what arrives at the renderer', () {
    test('is as long as the renderer expects', () {
      expect(OrblitPostProcess().packed, hasLength(OrblitPostProcess.stride));
    });

    test('starts with everything off but the picture still made', () {
      final post = OrblitPostProcess();
    ...
    test('carries every number it was given', () {
      final post = OrblitPostProcess(
        bloom: OrblitBloom(enabled: true, strength: 0.4, levels: 8),
        vignette: OrblitVignette(enabled: true, midPoint: 0.3),
    ...
    test('a colour triple is three numbers in order', () {
      ...
      final packed = post.packed;
      final at = OrblitPostProcess.stride - 9;
  ...
  group('the scene it belongs to', () {
    test('has one, and sends it', () {
      ...
      expect((message['postParams']! as List).length, OrblitPostProcess.stride);
    });
    ...
    test('a look belongs to the scene rather than to a camera', () {
      ...
      final scene = OrblitScene(
        ...
        post: OrblitPostProcess(bloom: OrblitBloom(enabled: true)),
      );
