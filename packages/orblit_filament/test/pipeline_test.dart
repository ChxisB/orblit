...
void main() {
  ...
  test('the four named settings differ only in how much, not in what', () {
    final tiers = [
      for (final detail in OrblitDetail.values) OrblitPipeline.at(detail),
    ];
  ...
  test('only the lowest setting is allowed to shrink the frame', () {
    expect(OrblitPipeline.at(OrblitDetail.low).resolution.adaptive, isTrue);
    for (final detail in [
      OrblitDetail.medium,
      OrblitDetail.high,
      OrblitDetail.ultra,
      ...
      expect(
        OrblitPipeline.at(detail).resolution.adaptive,
        isFalse,
  ...
  test('a fixed scale packs as a range with nowhere to go', () {
    final fixed = OrblitPipeline(resolution: OrblitResolution(scale: 0.75));
    ...
    final moving = OrblitPipeline(
      resolution: OrblitResolution(adaptive: true, minScale: 0.6, maxScale: 1),
    );
  ...
  test('the shadow flags pack together', () {
    final plain = OrblitPipeline().packed;
    expect(plain[8], 0);
    final both = OrblitPipeline(
      shadows: OrblitShadows(stable: true, contact: true),
    ).packed;
    expect(both[8], 3);
    final contactOnly = OrblitPipeline(
      shadows: OrblitShadows(contact: true),
    ).packed;
  ...
  test('the variance flags pack beside the older two', () {
    final packed = OrblitPipeline(
      shadows: OrblitShadows(
        stable: true,
        variance: OrblitVarianceShadows(
          highPrecision: true,
          mipmapping: true,
          exponential: true,
        ),
  ...
  test('a short split list fills only what it has', () {
    final packed = OrblitPipeline(
      shadows: OrblitShadows(cascades: 2, splits: [0.25]),
    ).packed;
    expect(packed.sublist(18, 21), [0.25, 0, 0]);
  });

  test('the view flags pack together', () {
    expect(OrblitPipeline().packed[15], 6, reason: 'culling and refraction');
    expect(
      OrblitPipeline(
        precise: true,
        culling: false,
  ...
  test('a scene carries a pipeline whether or not one was given', () {
    ...
    final ultra = OrblitScene(
      objects: const [],
      pipeline: OrblitPipeline.at(OrblitDetail.ultra),
      camera: OrblitCamera(position: Vector3(0, 0, 5), target: Vector3.zero()),
    );
  ...
  test('the shadow kind travels as its own index', () {
    for (final kind in OrblitShadowKind.values) {
      final packed = OrblitPipeline(shadows: OrblitShadows(kind: kind)).packed;
      expect(packed[1], kind.index);
    }
  });

  test('a handheld is 1280 by 800, and that is not 16:9', () {
    const deck = OrblitDisplay.handheld;
  ...
  test('a small panel gets a shadow map matched to it', () {
    ...
    final named = OrblitPipeline.at(OrblitDetail.high);
    final deck = OrblitPipeline.forDisplay(
      const OrblitDisplay(width: 1280, height: 800, detail: OrblitDetail.high),
    );
  ...
  test('the map never falls below the lowest setting the engine has', () {
    ...
    final deck = OrblitPipeline.forDisplay(
      const OrblitDisplay(width: 1280, height: 800, detail: OrblitDetail.low),
    );
  ...
  test('the floor under a shrinking frame rises on a small panel', () {
    ...
    expect(OrblitPipeline.at(OrblitDetail.low).resolution.minScale, 0.5);
  });

  test('multisampling comes off, since bandwidth is what it costs', () {
    for (final detail in [OrblitDetail.high, OrblitDetail.ultra]) {
      expect(OrblitPipeline.at(detail).samples, 4);
      expect(
        OrblitPipeline.forDisplay(
          OrblitDisplay(width: 1280, height: 800, detail: detail),
        ).samples,
        1,
  ...
  test('a large display is left exactly as the named setting had it', () {
    ...
    for (final detail in OrblitDetail.values) {
      final named = OrblitPipeline.at(detail);
      final big = OrblitPipeline.forDisplay(
        OrblitDisplay(width: 2560, height: 1440, detail: detail),
      );
  ...
  test('nothing appears or disappears on a handheld', () {
    ...
    for (final detail in OrblitDetail.values) {
      ...
      expect(deck.packed, hasLength(OrblitPipeline.stride));
    }
  });
}
