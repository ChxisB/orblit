...
void main() {
  const tree = OrblitLod([
    OrblitStep('/trees/oak_near.glb', 30),
    OrblitStep('/trees/oak_mid.glb', 120),
    OrblitStep('/trees/oak_far.glb', 400),
  ]);
  ...
  group('choosing a step', () {
    ...
    test('a step list that runs inwards is a mistake worth seeing', () {
      const muddled = OrblitLod([
        OrblitStep('/far.glb', 400),
        OrblitStep('/near.glb', 30),
      ]);
    ...
    test('one step is a range, not a level of detail, and that is allowed', () {
      const simple = OrblitLod([OrblitStep('/rock.glb', 80)]);
      expect(simple.meshAt(79), '/rock.glb');
      expect(simple.meshAt(81), isNull);
    });

    test('a last step that never ends draws for ever', () {
      const mountain = OrblitLod([
        OrblitStep('/near.glb', 500),
        OrblitStep('/far.glb', double.infinity),
      ]);
