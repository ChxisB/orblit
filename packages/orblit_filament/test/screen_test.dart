...
void main() {
  ...
  Float32List raysOf(OrblitScene scene) =>
  ...
  group('god rays', () {
    ...
    test('a strength of nought is off, whatever else is set', () {
      final scene = sceneWith(
        godRays: const OrblitGodRays(strength: 0, samples: 128),
      );
      expect(scene.passNames, ['scene']);
      expect(raysOf(scene), everyElement(0));
    });
    ...
    test('turned on, the world goes into a texture and the shafts onto '
        'the screen', () {
      final scene = sceneWith(godRays: const OrblitGodRays(strength: 0.8));
      final graph = scene.drawnGraph;
      expect(graph.problems, isEmpty);
      expect(scene.passNames, ['scene', 'god rays']);
      expect(graph.schedule.first.into, OrblitRenderGraph.screenTarget);
      final last = graph.schedule.last;
      expect(last.effect, OrblitEffect.godRays);
      expect(last.into, isNull);
      expect(last.reads, [OrblitRenderGraph.screenTarget]);
    });

    test('with no directional light there is nothing to scatter', () {
      final scene = sceneWith(
        godRays: const OrblitGodRays(strength: 1),
        sun: false,
      );
      expect(scene.passNames, ['scene']);
      expect(raysOf(scene), everyElement(0));
    ...
    test("come from the scene's own light, in its colour", () {
      final packed = raysOf(
        sceneWith(godRays: const OrblitGodRays(strength: 0.5, samples: 32)),
      );
    ...
    test('a tint of their own wins over the light', () {
      final packed = raysOf(
        sceneWith(
          godRays: OrblitGodRays(strength: 1, tint: Vector3(0.2, 0.4, 1)),
        ),
      );
      expect(packed.sublist(4, 7), [
        closeTo(0.2, 1e-6),
    ...
    test('cloud only thins them when the sky that carries it is drawn', () {
      final cloudy = OrblitSky(clouds: OrblitClouds.cumulus(cover: 0.7));
      expect(
        raysOf(
          sceneWith(godRays: const OrblitGodRays(strength: 1), sky: cloudy),
        )[10],
        closeTo(0.7, 1e-6),
      );
      final hidden = OrblitSky(
        drawn: false,
        clouds: OrblitClouds.cumulus(cover: 0.7),
      );
      expect(
        raysOf(
          sceneWith(godRays: const OrblitGodRays(strength: 1), sky: hidden),
        )[10],
        0,
      );
    });
    ...
    test('a god-ray pass needs a target that kept its depth', () {
      final graph = OrblitRenderGraph(
        ...
        passes: const [
          OrblitPass(name: 'world', into: 'flat'),
          OrblitPass(
            name: 'rays',
            kind: OrblitPassKind.effect,
  ...
  group('distortion', () {
    test('a wave that has run its course draws no pass', () {
      final done = OrblitDistortion.expanding(
        centre: Vector3.zero(),
        age: 3,
        lifetime: 2,
      );
      ...
      expect(OrblitDistortion.packAll([done]), isEmpty);
    });

    test('a wave grows with the clock and weakens as it goes', () {
      final early = OrblitDistortion.expanding(
        centre: Vector3.zero(),
        age: 0.5,
        speed: 4,
        lifetime: 2,
      ...
      final late = OrblitDistortion.expanding(
        centre: Vector3.zero(),
        age: 1.5,
        speed: 4,
        lifetime: 2,
    ...
    test('each kind packs into the slots the renderer reads', () {
      final packed = OrblitDistortion.packAll([
        OrblitDistortion.shockwave(
        ...
        OrblitDistortion.lens(strength: -0.1),
      ]);
      const s = OrblitDistortion.stride;
    ...
    test('no more than the renderer can draw are sent', () {
      final many = [
        for (var i = 0; i < 12; i++) OrblitDistortion.lens(strength: 0.01),
      ];
      expect(
        OrblitDistortion.packAll(many).length,
        OrblitDistortion.capacity * OrblitDistortion.stride,
      );
    });
    ...
    test(
      ...
      () {
        ...
        final scene = sceneWith(
          godRays: const OrblitGodRays(strength: 1),
          distortions: [OrblitDistortion.lens(strength: 0.1)],
        );
        final graph = scene.drawnGraph;
        expect(graph.problems, isEmpty);
        expect(scene.passNames, ['scene', 'god rays', 'distortion']);
        expect(graph.schedule[1].into, OrblitRenderGraph.raysTarget);
        expect(graph.schedule.last.reads, [
          OrblitRenderGraph.raysTarget,
          OrblitRenderGraph.screenTarget,
        ]);
      },
    );
  });
