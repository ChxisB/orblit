...
void main() {
  group('a reflection probe', () {
    OrblitScene sceneWith(List<OrblitProbe> probes) => OrblitScene(
      objects: const [],
    ...
    test('it crosses with everything the capture needs', () {
      ...
      expect(params, hasLength(OrblitProbe.stride));
    ...
    test('the version is what asks for a new photograph', () {
      ...
      List<double> paramsFor(int version) =>
          sceneWith([
                OrblitProbe(key: 1, position: Vector3.zero(), version: version),
    ...
    test('the probe count is the lattice multiplied out', () {
      expect(OrblitProbe(key: 1, position: Vector3.zero()).resolution, 256);
    });
  });

  group('a world-space irradiance field', () {
    OrblitScene sceneWith(OrblitField field) => OrblitScene(
      objects: const [],
      camera: OrblitCamera(position: Vector3(0, 0, 5), target: Vector3.zero()),
      field: field,
    );
    ...
    test('a scene that says nothing about one still sends its numbers', () {
      ...
      final params =
          sceneWith(OrblitField.none).toMessage(1)['fieldParams']!
              as List<double>;
      expect(params, hasLength(OrblitField.stride));
    ...
    test('it names the target it fills itself from', () {
      ...
      final message = sceneWith(
        OrblitField(enabled: true, from: 'lit'),
      ).toMessage(1);
      expect(message['fieldFrom'], 'lit');
    });

    test('the probe count is the lattice multiplied out', () {
      expect(OrblitField(counts: Vector3(5, 4, 6)).probeCount, 120);
    });
  });

  OrblitScene sceneWith(OrblitEnvironment? environment) => OrblitScene(
    objects: const [],
  ...
  group('an environment', () {
    test('nothing named is nothing set', () {
      expect(OrblitEnvironment.none.isSet, isFalse);
      expect(const OrblitEnvironment(radiance: '').isSet, isFalse);
    });
    ...
    test('either half on its own is a real environment', () {
      ...
      expect(const OrblitEnvironment(radiance: '/k_ibl.ktx').isSet, isTrue);
      expect(const OrblitEnvironment(skybox: '/k_skybox.ktx').isSet, isTrue);
    });

    test('it is stated in the same unit as every other light', () {
      const outdoors = OrblitEnvironment(
    ...
    test('turning it is what makes one bake serve any scene', () {
      const turned = OrblitEnvironment(radiance: '/a.ktx', rotation: 1.5);
    ...
    test('the backdrop can be sampled without being drawn', () {
      ...
      expect(const OrblitEnvironment(radiance: '/a.ktx').packed[2], 1);
    });
  });
  ...
  group('on a scene', () {
    test('a scene that says nothing sends an environment of nothing', () {
      ...
      expect(message['environmentParams'], hasLength(OrblitEnvironment.stride));
    });
    ...
    test('changing only the brightness leaves the files alone', () {
      ...
      const before = OrblitEnvironment(radiance: '/a.ktx', intensity: 1000);
