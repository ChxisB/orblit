...
OrblitScene _scene(List<OrblitDecal> decals) => OrblitScene(
  objects: const [],
...
void main() {
  group('a decal on the wire', () {
    test('packs every field where the renderer reads it', () {
      ...
      final packed = Float32List(OrblitDecal.stride);
      decal.pack(packed, 0);
    ...
    test('reaches every layer unless told otherwise', () {
      final everywhere = OrblitDecal(
        key: 1,
        position: Vector3.zero(),
        size: Vector3.all(1),
      );
      expect(everywhere.layerMask, 0x7F);
      final nowhere = OrblitDecal(
        key: 1,
        position: Vector3.zero(),
        size: Vector3.all(1),
        layers: {9, -1},
    ...
    test('sends each picture once and points at it', () {
      const poster = OrblitTexture('/tmp/poster.png');
      final message = _scene([
        ...
        OrblitDecal(key: 2, position: Vector3.zero(), size: Vector3.all(1)),
        OrblitDecal(
          key: 3,
          position: Vector3.zero(),
          size: Vector3.all(1),
    ...
    test('copyWith keeps the decals, and can replace them', () {
      final scene = _scene([
        OrblitDecal(key: 1, position: Vector3.zero(), size: Vector3.all(1)),
      ]);
