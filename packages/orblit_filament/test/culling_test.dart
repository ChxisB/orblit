...
void main() {
  OrblitBounds cubeAt(double x, double y, double z, [double half = 0.5]) =>
      OrblitBounds(
  ...
  group('a box', () {
    ...
    test('area is what the tree is built to minimise', () {
      ...
      expect(
        OrblitBounds(Vector3.zero(), Vector3(1, 2, 3)).area,
  ...
  group('a tree', () {
    ...
    List<OrblitVolume> line(int count) => [
      for (var i = 0; i < count; i++) OrblitVolume(i, cubeAt(i * 2.0, 0, 0)),
    ];

    test('an empty scene is an empty tree, not a broken one', () {
      final tree = OrblitBvh.of(const []);
    ...
    test('it holds everything it was given', () {
      final tree = OrblitBvh.of(line(100));
    ...
    test('a box query finds exactly what overlaps it', () {
      ...
      final found = tree.inside(
        OrblitBounds(Vector3(-1, -1, -1), Vector3(5, 1, 1)),
      )..sort();
    ...
    test('a ray finds what it passes through, nearest first', () {
      final tree = OrblitBvh.of(line(20));
    ...
    test('the answer is the same as asking everything one at a time', () {
      ...
      final tree = OrblitBvh.of(volumes);
      final query = OrblitBounds(Vector3(-20, -20, -20), Vector3(20, 20, 20));
  ...
  group('the frustum', () {
    ...
    test('what is inside is kept and what is outside is not', () {
      final tree = OrblitBvh.of([
        OrblitVolume(1, cubeAt(0, 0, 0)),
        OrblitVolume(2, cubeAt(5, 0, 0)),
        OrblitVolume(3, cubeAt(100, 0, 0)),
      ]);
    ...
    test('a box straddling a plane is kept', () {
      ...
      final tree = OrblitBvh.of([OrblitVolume(1, cubeAt(10, 0, 0, 2))]);
