...
void main() {
  OrblitScene courtyard({
    ...
    sky: OrblitSky(colour: Vector3(0.6, 0.7, 0.9), ambient: 20000),
    fog: OrblitFog(density: 0.002, colour: Vector3(0.7, 0.8, 0.9)),
    post: post,
  ...
  OrblitEnvironmentVolume hall({
    OrblitEnvironmentOverrides? overrides,
    int priority = 0,
    int key = 1,
    double weight = 1,
  ...
  group('where a volume reaches', () {
    test('full inside, nothing past the blend, a smoothstep between', () {
      final sphere = OrblitEnvironmentVolume.sphere(
        key: 1,
        centre: Vector3.zero(),
        radius: 3,
        blendDistance: 2,
    ...
    test('a hard edge is all or nothing', () {
      final hard = OrblitEnvironmentVolume.sphere(
        key: 1,
        centre: Vector3.zero(),
        radius: 1,
        blendDistance: 0,
  ...
  group('resolving the look', () {
    ...
    test('what the volume does not name is left exactly alone', () {
      final scene = courtyard(
        volumes: [hall(overrides: const OrblitEnvironmentOverrides())],
        camera: Vector3.zero(),
      );
      final seen = scene.resolved();
      expect(identical(seen.fog, scene.fog), isTrue);
      ...
      final fogOnly = courtyard(
        volumes: [
          hall(overrides: const OrblitEnvironmentOverrides(fogDensity: 0.3)),
        ],
        camera: Vector3.zero(),
      );
    ...
    test('a turn blends the short way round', () {
      final scene = courtyard(
        environment: const OrblitEnvironment(rotation: 350 * math.pi / 180),
        volumes: [
          hall(
            overrides: const OrblitEnvironmentOverrides(
    ...
    test('a switched-off bloom and grade come on from nothing', () {
      final scene = courtyard(
        volumes: [
          hall(
            overrides: OrblitEnvironmentOverrides(
              bloomStrength: 0.4,
              saturation: 0.5,
              temperature: 0.2,
    ...
    test('where two overlap, the higher priority has the last word', () {
      final low = hall(
        key: 1,
        overrides: const OrblitEnvironmentOverrides(fogDensity: 0.1),
      );
      final high = hall(
        key: 2,
        priority: 5,
        overrides: const OrblitEnvironmentOverrides(fogDensity: 0.5),
      );
      for (final order in [
        [low, high],
        [high, low],
    ...
    test('the same question always gets the same answer', () {
      ...
      final base = OrblitEnvironmentSettings.of(courtyard());
  ...
  group('on the wire', () {
    ...
    test('copying a scene keeps its probes, field and volumes', () {
      final scene = OrblitScene(
        ...
        probes: [OrblitProbe(key: 9, position: Vector3.zero())],
        volumes: [hall()],
      );
      final copy = scene.copyWith(fog: OrblitFog.none);
