...
DARWIN_NATIVE="../darwin/orblit_filament/Sources/orblit_filament_native"
GENERATED="$DARWIN_NATIVE/generated"

fail() { echo "orblit_filament/linux/setup.sh: $*" >&2; exit 1; }
...
# "linux" and unpacks to lib/x86_64. ORBLIT_FILAMENT_ARCH overrides the
...
if [ -d "$FILAMENT_LINUX_DIR/include" ]; then
  ...
  echo "orblit_filament/linux: third_party/filament -> $FILAMENT_LINUX_DIR"
elif [ -d "$FILAMENT/include" ]; then
  echo "orblit_filament/linux: Filament $FILAMENT_VERSION ($FLAVOUR) already present"
else
  echo "orblit_filament/linux: fetching Filament $FILAMENT_VERSION ($FLAVOUR)"
  ...
  curl -fsSL -o /tmp/orblit_ltc_linux.js "$LTC_FROM"
  {
    ...
    slice() {
      ...
      awk -v want="$1" '
        ...
        { print line }
      ' /tmp/orblit_ltc_linux.js
      echo "};"
    }
    slice g_ltc_1 kLtcMatrix
    slice g_ltc_2 kLtcFresnel
  } > "$LTC_HEADER"
  rm -f /tmp/orblit_ltc_linux.js
fi

# 3. The materials themselves. Vulkan first because that is what
#    orblit::backendCandidates asks for first off Apple, and OpenGL because
...
ORBLIT_MATC_BACKENDS="${ORBLIT_MATC_BACKENDS:-vulkan opengl}"
MATC_API=""
for api in ${ORBLIT_MATC_BACKENDS//,/ }; do
  case "$api" in
    metal|vulkan|opengl|all) MATC_API="$MATC_API -a $api" ;;
    *)
      fail "ORBLIT_MATC_BACKENDS names '$api', which is not one of metal," \
        "vulkan, opengl or all."
      ;;
  esac
done
MATC_PROFILE="${ORBLIT_MATC_PROFILE:-desktop}"
...
# lit_slim is the nine-sampler surface orblit::Renderer chooses instead of lit
...
compile() {
  ...
  echo "orblit_filament/linux: compiling $name"
  local input="$source"
  if [ -n "$blend" ]; then
    input="/tmp/orblit_linux_src_$name.mat"
    sed "s/^\( *blending *: *\)[a-z]*,/\1$blend,/" "$source" > "$input"
  fi
  # shellcheck disable=SC2086 -- the flags are ours and are meant to split.
  "$MATC" $MATC_FLAGS -o "/tmp/orblit_linux_$name.filamat" "$input"
  ...
  # the same.
  {
    ...
    od -An -v -tx1 "/tmp/orblit_linux_$name.filamat" \
      | sed -e 's/[0-9a-f][0-9a-f]/0x&,/g' -e 's/^ */  /'
    echo "};"
    echo "unsigned int k${name}Material_len = $(wc -c < "/tmp/orblit_linux_$name.filamat" | tr -d ' ');"
  } > "$header"
  rm -f "/tmp/orblit_linux_$name.filamat" "/tmp/orblit_linux_src_$name.mat"
}
...
echo "orblit_filament/linux: setup complete"
