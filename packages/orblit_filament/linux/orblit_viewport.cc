...
G_DECLARE_FINAL_TYPE(OrblitFrameTexture,
                     orblit_frame_texture,
                     ORBLIT,
                     FRAME_TEXTURE,
                     FlPixelBufferTexture)

struct _OrblitFrameTexture {
  FlPixelBufferTexture parent_instance;
  orblit_linux::Viewport* viewport;
};

G_DEFINE_TYPE(OrblitFrameTexture,
              orblit_frame_texture,
              fl_pixel_buffer_texture_get_type())

// Flutter's render thread, not the main one. Everything it touches is
// behind the viewport's own lock.
static gboolean orblit_frame_texture_copy_pixels(FlPixelBufferTexture* texture,
                                                const uint8_t** buffer,
                                                uint32_t* width,
                                                uint32_t* height,
                                                GError** error) {
  auto* self = ORBLIT_FRAME_TEXTURE(texture);
  if (self->viewport == nullptr ||
    ...
    // capture arrives.
    g_set_error(error, g_quark_from_static_string("orblit-filament"), 0,
                "no frame yet");
    return FALSE;
  }
  return TRUE;
...
static void orblit_frame_texture_class_init(OrblitFrameTextureClass* klass) {
  FL_PIXEL_BUFFER_TEXTURE_CLASS(klass)->copy_pixels =
      orblit_frame_texture_copy_pixels;
}

static void orblit_frame_texture_init(OrblitFrameTexture* self) {}

// ---- A PNG, written without a library ----------------------------------
//
// The same stored-block deflate native/headless/orblit_headless.c writes, in
...
std::unique_ptr<Viewport> Viewport::Start(FlTextureRegistrar* registrar,
  ...
  // Offscreen and readable. ORBLIT_BACKEND_DEFAULT is the platform's own
  // choice -- orblit::backendCandidates asks for Vulkan and falls back to
  // OpenGL off Apple -- or whatever ORBLIT_BACKEND names.
  orblit_surface_desc surface{};
  surface.kind = ORBLIT_SURFACE_HEADLESS;
  surface.window = nullptr;
  viewport->renderer_ = orblit_renderer_create(ORBLIT_BACKEND_DEFAULT, &surface,
                                              width, height);
  if (viewport->renderer_ == nullptr) return nullptr;

  auto* texture = ORBLIT_FRAME_TEXTURE(
      g_object_new(orblit_frame_texture_get_type(), nullptr));
...
bool Viewport::OnFrame() {
  ...
  orblit_renderer_request_capture(renderer_);
  orblit_renderer_draw(renderer_, seconds);
  frame_count_++;

  uint32_t width = 0;
  uint32_t height = 0;
  const size_t bytes =
      orblit_renderer_read_capture(renderer_, nullptr, 0, &width, &height);
  ...
  slot.resize(bytes);
  orblit_renderer_read_capture(renderer_, slot.data(), slot.size(), &width,
                              &height);
  {
...
void Viewport::WriteFrameIfAsked(const uint8_t* rgba, uint32_t width,
                                 uint32_t height) {
  const char* wanted = std::getenv("ORBLIT_DUMP_FRAME");
...
void Viewport::Resize(uint32_t width, uint32_t height) {
  ...
  orblit_renderer_resize(renderer_, width, height);
}
...
std::vector<std::pair<std::string, std::string>> Viewport::Notes() const {
  ...
  const uint32_t count = orblit_renderer_notes(renderer_);
  out.reserve(count);
  for (uint32_t i = 0; i < count; i++) {
    const char* about = nullptr;
    const char* saying = nullptr;
    if (orblit_renderer_note(renderer_, i, &about, &saying) != ORBLIT_OK) {
      continue;
    }
...
double Viewport::GpuMilliseconds() const {
  ...
  orblit_stats stats{};
  if (orblit_renderer_stats(renderer_, &stats) != ORBLIT_OK) return 0;
...
std::vector<double> Viewport::PassTimings() const {
  ...
  const uint32_t count =
      orblit_renderer_pass_timings(renderer_, nullptr, nullptr, 0);
  std::vector<double> milliseconds(count);
  std::vector<int32_t> drawn(count);
  orblit_renderer_pass_timings(renderer_, milliseconds.data(), drawn.data(),
                              count);
...
std::vector<int32_t> Viewport::Batching() const {
  ...
  orblit_stats stats{};
  if (orblit_renderer_stats(renderer_, &stats) != ORBLIT_OK) return {0, 0};
