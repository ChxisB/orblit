#include "include/orblit_filament/orblit_filament_plugin.h"
...
// Linux's `orblit_filament` plugin: the same channel, the same methods, the
// same wire shapes as OrblitFilamentPlugin.swift and OrblitFilamentPlugin.kt,
// so lib/src/orblit_view.dart and every other Dart caller work unchanged. One
...
// include/orblit_filament/orblit_filament_plugin.h has to offer.
G_DECLARE_FINAL_TYPE(OrblitFilamentPlugin,
                     orblit_filament_plugin,
                     ORBLIT,
                     FILAMENT_PLUGIN,
                     GObject)
...
G_DEFINE_TYPE(OrblitFilamentPlugin, orblit_filament_plugin, g_object_get_type())
...
orblit_linux::Viewport* Find(OrblitFilamentPlugin* self, int64_t texture_id) {
  auto found = self->viewports->find(texture_id);
  return found == self->viewports->end() ? nullptr : found->second.get();
}

FlMethodResponse* Create(OrblitFilamentPlugin* self, FlValue* args) {
  int64_t width = 0;
  int64_t height = 0;
  ...
  std::unique_ptr<orblit_linux::Viewport> viewport =
      orblit_linux::Viewport::Start(self->texture_registrar, uint32_t(width),
                                   uint32_t(height));
  if (viewport == nullptr) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "no-renderer",
...
FlMethodResponse* Resize(OrblitFilamentPlugin* self, FlValue* args) {
  ...
  orblit_linux::Viewport* viewport = Find(self, texture_id);
  if (viewport != nullptr) {
...
FlMethodResponse* SetScene(OrblitFilamentPlugin* self, FlValue* args) {
  int64_t texture_id = 0;
  if (!ReadInt(args, "textureId", &texture_id)) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "bad-args", "setScene needs a textureId", nullptr));
  }
  std::unique_ptr<orblit_linux::Scene> scene = orblit_linux::Scene::From(args);
  if (scene == nullptr) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
        "bad-scene",
...
FlMethodResponse* Stats(OrblitFilamentPlugin* self, FlValue* args) {
  int64_t texture_id = 0;
...
void MethodCall(FlMethodChannel* channel, FlMethodCall* method_call,
                gpointer user_data) {
  auto* self = ORBLIT_FILAMENT_PLUGIN(user_data);
  const gchar* method = fl_method_call_get_name(method_call);
  FlValue* args = fl_method_call_get_args(method_call);
...
static void orblit_filament_plugin_dispose(GObject* object) {
  auto* self = ORBLIT_FILAMENT_PLUGIN(object);
...
static void orblit_filament_plugin_class_init(OrblitFilamentPluginClass* klass) {
  G_OBJECT_CLASS(klass)->dispose = orblit_filament_plugin_dispose;
}

static void orblit_filament_plugin_init(OrblitFilamentPlugin* self) {
  self->viewports =
      new std::map<int64_t, std::unique_ptr<orblit_linux::Viewport>>();
}

void orblit_filament_plugin_register_with_registrar(
    FlPluginRegistrar* registrar) {
  auto* plugin = ORBLIT_FILAMENT_PLUGIN(
      g_object_new(orblit_filament_plugin_get_type(), nullptr));

  plugin->texture_registrar = fl_plugin_registrar_get_texture_registrar(registrar);

  g_autoptr(FlStandardMethodCodec) codec = fl_standard_method_codec_new();
  plugin->channel = fl_method_channel_new(
      fl_plugin_registrar_get_messenger(registrar), "orblit_filament",
