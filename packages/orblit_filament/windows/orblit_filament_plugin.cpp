...
// Windows's `orblit_filament` plugin: the same channel, the same methods, the
// same wire shapes as OrblitFilamentPlugin.swift, OrblitFilamentPlugin.kt and
// ../linux/orblit_filament_plugin.cc, so lib/src/orblit_view.dart and every
...
class OrblitFilamentPlugin : public flutter::Plugin {
 public:
  ...
  explicit OrblitFilamentPlugin(flutter::TextureRegistrar* textures)
      : textures_(textures) {}

  ~OrblitFilamentPlugin() override = default;

  OrblitFilamentPlugin(const OrblitFilamentPlugin&) = delete;
  OrblitFilamentPlugin& operator=(const OrblitFilamentPlugin&) = delete;
...
void OrblitFilamentPlugin::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows* registrar) {
  auto channel = std::make_unique<flutter::MethodChannel<EncodableValue>>(
      registrar->messenger(), "orblit_filament",
      &flutter::StandardMethodCodec::GetInstance());

  auto plugin =
      std::make_unique<OrblitFilamentPlugin>(registrar->texture_registrar());
...
void OrblitFilamentPlugin::HandleMethodCall(
...
void OrblitFilamentPlugin::Create(const EncodableValue* args,
                                 std::unique_ptr<MethodResultValue> result) {
  int64_t width = 0;
  int64_t height = 0;
...
void OrblitFilamentPlugin::Resize(const EncodableValue* args,
                                 std::unique_ptr<MethodResultValue> result) {
...
void OrblitFilamentPlugin::SetScene(const EncodableValue* args,
                                   std::unique_ptr<MethodResultValue> result) {
  int64_t texture_id = 0;
  if (!ReadInt(args, "textureId", &texture_id)) {
...
void OrblitFilamentPlugin::Stats(const EncodableValue* args,
                                std::unique_ptr<MethodResultValue> result) {
  int64_t texture_id = 0;
  Viewport* viewport =
...
void OrblitFilamentPlugin::Dispose(const EncodableValue* args,
                                  std::unique_ptr<MethodResultValue> result) {
  int64_t texture_id = 0;
  if (!ReadInt(args, "textureId", &texture_id)) {
...
void OrblitFilamentPluginRegisterWithRegistrar(
    FlutterDesktopPluginRegistrarRef registrar) {
  orblit_windows::OrblitFilamentPlugin::RegisterWithRegistrar(
