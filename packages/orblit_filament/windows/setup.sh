...
#        deliberate, not a shortcut: OrblitRendererCore.cpp is one file
...
fail() { echo "orblit_filament/windows/setup.sh: $*" >&2; exit 1; }
...
if [ -d "$FILAMENT/include" ]; then
  echo "orblit_filament/windows: Filament $FILAMENT_VERSION already present"
elif [ "$HOST" != "windows" ] && [ -d "$FILAMENT_WINDOWS_DIR/include" ]; then
  ln -sfn "$FILAMENT_WINDOWS_DIR" "$FILAMENT"
  echo "orblit_filament/windows: third_party/filament -> $FILAMENT_WINDOWS_DIR"
else
  echo "orblit_filament/windows: fetching Filament $FILAMENT_VERSION (windows)"
  ...
  curl -fsSL -o /tmp/orblit_ltc_windows.js "$LTC_FROM"
  {
    ...
    slice() {
      ...
      awk -v want="$1" '
        ...
        { print line }
      ' /tmp/orblit_ltc_windows.js
      echo "};"
    }
    slice g_ltc_1 kLtcMatrix
    slice g_ltc_2 kLtcFresnel
  } > "$LTC_HEADER"
  rm -f /tmp/orblit_ltc_windows.js
fi
...
compile() {
  ...
  echo "orblit_filament/windows: compiling $name"
  local input="$source"
  if [ -n "$blend" ]; then
    input="/tmp/orblit_windows_src_$name.mat"
    sed "s/^\( *blending *: *\)[a-z]*,/\1$blend,/" "$source" > "$input"
  fi
  # shellcheck disable=SC2086 -- the flags are ours and are meant to split.
  "$MATC" $MATC_FLAGS -o "/tmp/orblit_windows_$name.filamat" "$input"
  ...
  # not ship xxd -- the output is what matters, and it is the same.
  {
    ...
    od -An -v -tx1 "/tmp/orblit_windows_$name.filamat" \
      | sed -e 's/[0-9a-f][0-9a-f]/0x&,/g' -e 's/^ */  /'
    echo "};"
    echo "unsigned int k${name}Material_len = $(wc -c < "/tmp/orblit_windows_$name.filamat" | tr -d ' ');"
  } > "$header"
  rm -f "/tmp/orblit_windows_$name.filamat" "/tmp/orblit_windows_src_$name.mat"
}
...
echo "orblit_filament/windows: setup complete"
