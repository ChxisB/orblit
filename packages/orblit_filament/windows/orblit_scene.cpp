...
double ReadNumber(const EncodableMap* map, const char* key, double fallback) {
  ...
  // translation unit through orblit_viewport.h in the same target and drags in
...
std::unique_ptr<Scene> Scene::From(const flutter::EncodableValue* args) {
  ...
  scene->pose_joint_counts_ = ReadInts(map, "poseJointCounts");
  scene->pose_joints_ = ReadInts(map, "poseJoints");
  scene->pose_joint_transforms_ = ReadFloats(map, "poseJointTransforms");

  return scene;
}

void Scene::ApplyTo(orblit_renderer* renderer) const {
