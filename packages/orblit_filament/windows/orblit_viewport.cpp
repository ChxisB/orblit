...
// the engine does.
constexpr const wchar_t kTimerWindowClass[] = L"OrblitFilamentFrameTimer";

// ---- A PNG, written without a library ----------------------------------
//
// The same stored-block deflate native/headless/orblit_headless.c writes, and
// that ../linux/orblit_viewport.cc carries: a PNG needs no compressor if every
...
void Viewport::OnFrame() {
  ...
  pixels.resize(bytes);
  orblit_renderer_read_capture(renderer_, pixels.data(), pixels.size(), &width,
                              &height);
  {
    std::lock_guard<std::mutex> guard(shared_->lock);
...
void Viewport::WriteFrameIfAsked(const uint8_t* rgba, uint32_t width,
  ...
  if (_dupenv_s(&wanted, &wanted_length, "ORBLIT_DUMP_FRAME") != 0 ||
      wanted == nullptr) {
    return;
  }
