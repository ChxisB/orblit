#!/usr/bin/env python3
"""fix.py <OrblitRendererCore.cpp> <replacements.cpp> — the hand conversions."""
import re
import sys
...
sub('"[orblit] frame %d: cpu', '"[orblit] frame %llu: cpu')
...
resub(r'"\[orblit\] %@: %zu files decoded in %\.0f ms",(\s*)_loadingName\.lastPathComponent,',
      r'"[orblit] %s: %zu files decoded in %.0f ms",\1orblit::lastPathComponent(_loadingName).c_str(),')
sub('''  drawOutline();
  _renderer->endFrame();
''', '''  drawOutline();
...
// OrblitDecals.cpp, which has nothing Apple in it. What is here is the part
...
while True:
    ...
    src = src[:m.start()] + 'orblit::format(' + inner + ')' + src[close + 1:]
