#!/usr/bin/env python3
"""convert.py <OrblitRenderer.mm> <methods out> <declarations out> <prelude out>

Turns the Objective-C implementation into orblit::Renderer member functions,
in the same order and with the same comments:

  - (T)foo:(A)a bar:(B)b {     ->   T Renderer::foo(A a, B b) {
  [self foo:x bar:y]           ->   foo(x, y)
  [NSString stringWithFormat:] ->   orblit::format(...)
  [self](...) captures         ->   [this](...)
...
prelude_to = next(i for i, l in enumerate(lines) if l.startswith('@interface OrblitRenderer ()'))
...
def convert_sends(s):
    ...
    while True:
        ...
        if pat == '[self ':
            ...
            replacement = f'orblit::format({inner.strip()})'
...
def keywords(s):
    ...
    s = re.sub(r'\bCFAbsoluteTimeGetCurrent\(\)', 'orblit::now()', s)
    ...
    s = re.sub(r'\bNSLog\(@"', 'orblit::log("', s)
    s = re.sub(r'(?<![\w@])@"', '"', s)
    return s
