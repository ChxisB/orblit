 ...
 *   this build actually runs (see OrblitBackend.cpp: __EMSCRIPTEN__ asks for
 *   ORBLIT_BACKEND_OPENGL, which is WebGL 2 in a browser) — implements
 ...
 *
 * So the "window" a web host hands ORBLIT_SURFACE_WINDOW through
 * orblit_surface_desc is that selector — "#canvas", say — not a real native
 * handle, and this surface's whole job is to keep it alive and pass it
 * through unchanged. ORBLIT_SURFACE_HEADLESS has nowhere to go on this
 ...
 * path (see orblit_renderer_create in OrblitRendererC.cpp) rather than a
 * crash — a real limitation of PlatformWebGL today, not a bug introduced
 * here, and worth knowing before reaching for orblit_renderer_request_capture
 * on the web build.
 *
 * Kept beside the web build rather than in the shared source tree: it reads
 * like OrblitSurfaceHeadless.cpp's SharedChainSurface because the same single
 * "one shared chain, however many slots" shape is correct here too, but the
 * two must not both define OrblitCreateWindowSurface/OrblitCreateHeadlessSurface
...
class WebCanvasSurface final : public OrblitSurface {
 public:
  ...
  // ORBLIT_DUMP_FRAME has no sandboxed-writable path in a browser; a host
...
OrblitSurface *OrblitCreateHeadlessSurface() {
  return new WebCanvasSurface(std::string());
}

OrblitSurface *OrblitCreateWindowSurface(void *window) {
  // A C string selector, e.g. "#canvas" — orblit_web_host.cpp's
  // orblit_web_create_on_canvas is what a JavaScript host actually calls, and
