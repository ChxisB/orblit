 ...
 *
 * orblit_renderer.h is not changed for this: orblit_renderer_create takes an
 * orblit_surface_desc by pointer, and building one from JavaScript means
 ...
 * actually needs, ORBLIT_SURFACE_WINDOW onto a canvas selector string. This
 ...
 *
 * A C ABI itself, same rules as orblit_renderer.h: a NULL handle in is a NULL
...
extern "C" orblit_renderer *orblit_web_create_on_canvas(OrblitBackend backend,
                                                       const char *selector,
                                                       uint32_t width,
                                                       uint32_t height) {
  ...
  return orblit_renderer_create(backend, &surface, width, height);
}
