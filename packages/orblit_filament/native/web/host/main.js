...
// binding, only Emscripten's ccall/cwrap over the same orblit_renderer.h
// every other host drives. Publishes the same scene
// native/headless/orblit_headless.c does -- a ground, five blocks and a sun
...
(() => {
  ...
  const ORBLIT_OK = 0;
  const ORBLIT_BACKEND_OPENGL = 3; // WebGL 2, the only backend this build's
                                   // materials were compiled for.
  const ORBLIT_SURFACE_WINDOW = 1;

  const statusEl = document.getElementById('status');
  const log = (...args) => {
    console.log('orblitWeb:', ...args);
  };
  ...
  // orblit_headless.c's place(), the same sixteen floats in the same order.
  ...
  function readNote(Module, renderer, index) {
    ...
    try {
      const ok = Module.ccall(
        'orblit_renderer_note', 'number',
        ['number', 'number', 'number', 'number'],
        [renderer, index, outPtr, outPtr + 4]);
      if (ok !== ORBLIT_OK) return null;
  ...
  function publishScene(Module, renderer) {
    ...
    Module.ccall('orblit_renderer_set_sky_colour', 'number',
      ['number', 'number', 'number', 'number'],
    ...
    Module.ccall('orblit_renderer_apply_lights', 'number',
    ...
    // orblit_headless.c's kObjects loop builds.
    const blocks = [
      [-1.8, 0.0, 0.2, 0.5],
      [-0.6, 0.25, -1.0, 0.75],
      [0.7, -0.1, 0.6, 0.4],
    ...
    const objectsOk = Module.ccall('orblit_renderer_apply_objects', 'number', [
      'number', 'number', 'number', 'number', 'number', 'number', 'number',
    ...
    Module.ccall('orblit_renderer_set_camera', 'number',
      ['number', 'number', 'number', 'number', 'number', 'number', 'number'],
      [renderer, eyePtr, lookPtr, 42.0, 0, 10.0, 0.0]);

    Module.ccall('orblit_renderer_set_exposure', 'number',
      ['number', 'number', 'number', 'number'],
      [renderer, 16.0, 1.0 / 125.0, 100.0]);

    freeAll(Module, toFree);
    return objectsOk === ORBLIT_OK;
  }
  ...
  async function main() {
    ...
    let Module;
    try {
      Module = await OrblitRendererModule({ canvas });
    } catch (err) {
    ...
    const renderer = Module.ccall(
      'orblit_web_create_on_canvas', 'number',
      ['number', 'string', 'number', 'number'],
      [ORBLIT_BACKEND_OPENGL, '#canvas', canvas.width, canvas.height]);

    if (renderer === 0) {
      setStatus(
        'the renderer would not start on this canvas -- see the console ' +
        'for what Filament refused and why.', 'bad');
      log('orblit_renderer_create failed (see the [orblit] lines above)');
      return;
    }
    ...
    const backend = Module.ccall('orblit_renderer_backend', 'number', ['number'], [renderer]);
    const noteCount = Module.ccall('orblit_renderer_notes', 'number', ['number'], [renderer]);
    const notes = [];
    ...
    // on-screen verification: orblit_renderer_request_capture/read_capture
    // go through filament::Renderer::readPixels against whatever the swap
    // chain's default render target is. ORBLIT_SURFACE_HEADLESS cannot
    // allocate on this backend (see OrblitSurfaceWeb.cpp), but this is a
    // ORBLIT_SURFACE_WINDOW canvas chain, which is a different question --
    ...
    let frames = 0;
    const draw = (nowMs) => {
      Module.ccall('orblit_renderer_draw', 'number', ['number', 'number'],
        [renderer, nowMs / 1000.0]);
      frames++;
      if (frames === 3) {
        Module.ccall('orblit_renderer_request_capture', 'number', ['number'], [renderer]);
      }
      if (!captureChecked && frames === 6) {
        captureChecked = true;
        const outPtr = Module._malloc(8);
        const bytes = Module.ccall('orblit_renderer_read_capture', 'number',
    ...
    requestAnimationFrame(() => {
      ...
      const lines = [
        `backend: ${backendName(backend)} (orblit_renderer_backend)`,
  ...
  main().catch((err) => {
    console.error('orblitWeb: fatal', err);
