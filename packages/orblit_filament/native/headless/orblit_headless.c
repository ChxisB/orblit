 ...
 *
 *   orblit_headless [out.png] [metal|vulkan|opengl|webgpu]
 *   orblit_headless out.png backend --benchmark frames warmup [width height]
 */
...
static int measure(orblit_renderer *renderer, uint32_t frames, uint32_t warmup,
                   benchmark_result *result) {
  double *costs = malloc((size_t)frames * sizeof(double));
  if (!costs) return 0;
  uint64_t previous = orblit_renderer_rendered_frames(renderer);
  ...
  for (uint64_t attempt = 0; attempt < target * 100 + 1000; attempt++) {
    ...
    if (orblit_renderer_draw(renderer, (double)completed / 60.0) != ORBLIT_OK) {
      free(costs);
      return 0;
    }
    const double after = milliseconds();
    const uint64_t rendered = orblit_renderer_rendered_frames(renderer);
    ...
    if (completed == target) {
      ...
      if (orblit_renderer_stats(renderer, &result->stats) != ORBLIT_OK) {
        free(costs);
        return 0;
      }
...
int main(int argc, char **argv) {
  ...
  const char *out = argc > 1 ? argv[1] : "orblit_headless.png";
  const OrblitBackend asked = backend_named(argc > 2 ? argv[2] : NULL);
  uint32_t width = 960, height = 540, frames = 0, warmup = 0;
  if ((argc > 2 && asked == ORBLIT_BACKEND_DEFAULT) ||
      (argc > 3 &&
  ...
  orblit_surface_desc surface = {ORBLIT_SURFACE_HEADLESS, NULL};
  const double created_from = milliseconds();
  orblit_renderer *renderer =
      orblit_renderer_create(asked, &surface, width, height);
  if (renderer == NULL) {
    /* Not named here: ORBLIT_BACKEND may have chosen for DEFAULT, and the
  ...
  printf("drawing with %s\n", backend_name(orblit_renderer_backend(renderer)));
  ...
  const int prepass = number_from("ORBLIT_PREPASS", 0);
  orblit_renderer_set_depth_prepass(renderer, prepass);
  ...
  const int slabs = number_from("ORBLIT_SLABS", 0);

  /* The sky it stands under, which is also what lights the shadows. */
  const float sky[3] = {0.30f, 0.45f, 0.70f};
  orblit_renderer_set_sky_colour(renderer, sky, slabs > 0 ? 9000.0f : 24000.0f,
                                slabs > 0 ? 0 : 1);
  ...
  orblit_renderer_apply_lights(renderer, light_count, light_keys, light_kinds,
  ...
  if (orblit_renderer_apply_objects(renderer, (uint32_t)count, keys, transforms,
                                   (size_t)count * 16, colours,
                                   (size_t)count * 3, meshes, flags, materials,
                                   shapes, NULL, 0, NULL, 0) != ORBLIT_OK) {
  ...
  orblit_renderer_set_camera(renderer, slabs > 0 ? slabs_eye : blocks_eye, look,
                            42.0f, 0, 10.0f, 0.0);
  ...
  orblit_renderer_read_capture(renderer, pixels, bytes, &got_width,
                              &got_height);
