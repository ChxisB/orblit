...
# ORBLIT_PLATFORM_PORTABLE, every *.cpp beside the renderer, no Objective-C.
...
#
#   ORBLIT_FILAMENT_SDK   a Linux Filament release (the directory holding
#                        include/ and lib/), required
#   ORBLIT_FILAMENT_ARCH  which slice of lib/ to link (default: uname -m)
#   ORBLIT_BUILD_DIR      where the objects and programs go (default: build)
#
# The materials must have been compiled for the backends this will run —
# ORBLIT_MATC_BACKENDS="vulkan opengl" ./darwin/setup.sh — or Filament starts,
...
for source in "$SRC"/*.cpp; do
  ...
  clang++ -std=c++17 -O2 -DORBLIT_PLATFORM_PORTABLE "${STDLIB[@]}" \
