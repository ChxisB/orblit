...
```sh
ORBLIT_FILAMENT_WEBGPU_SRC=/absolute/path/to/orblit-filament \
ORBLIT_MATC_BACKENDS=all \
ORBLIT_BUILD_DIR=/tmp/orblit-all-api-build \
bash packages/orblit_filament/native/webgpu/build.sh

/tmp/orblit-all-api-build/orblit_headless /tmp/metal.png metal --benchmark 600 120 1280 720
/tmp/orblit-all-api-build/orblit_headless /tmp/webgpu.png webgpu --benchmark 600 120 1280 720
/tmp/orblit-all-api-build/orblit_headless /tmp/opengl.png opengl --benchmark 600 120 1280 720
```
...
The [headless host](headless/orblit_headless.c) also accepts `vulkan` and
...
returns top-first rows; Orblit now preserves that ordering. The C ABI test's
