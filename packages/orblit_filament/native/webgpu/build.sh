...
#
#   ORBLIT_FILAMENT_WEBGPU_SRC=/path/to/orblit-filament ./build.sh test
#   ORBLIT_MATC_BACKENDS=all compiles Metal, Vulkan, OpenGL and WebGPU into
...
FORK="${ORBLIT_FILAMENT_WEBGPU_SRC:-}"
if [ -z "$FORK" ]; then
  echo "native/webgpu/build.sh: ORBLIT_FILAMENT_WEBGPU_SRC is not set" >&2
  echo "  point it at the matching orblit-filament checkout" >&2
  exit 1
fi
case "$FORK" in
  /*) ;;
...
MATC="${ORBLIT_FILAMENT_WEBGPU_MATC:-$FORK/out/cmake-webgpu-release/tools/matc/matc}"
if [ -z "${ORBLIT_FILAMENT_WEBGPU_MATC:-}" ] && [ ! -x "$MATC" ]; then
...
API="${ORBLIT_MATC_BACKENDS:-webgpu}"
case "$API" in
  webgpu) GENERATED_SET=webgpu ;;
  all) GENERATED_SET=webgpu-all ;;
  *)
    echo "native/webgpu/build.sh: ORBLIT_MATC_BACKENDS must be webgpu or all" >&2
    exit 1
    ;;
esac
...
for table in AreaTex.h SearchTex.h LtcTables.h; do
  if [ ! -s "$GENERATED/$table" ]; then
    if [ ! -s "$COMMON/$table" ]; then
      ...
      echo "  run packages/orblit_filament/darwin/setup.sh first" >&2
      exit 1
    fi
...
MATERIAL_TMP="$(mktemp -d "${TMPDIR:-/tmp}/orblit-webgpu.XXXXXX")"
cleanup() {
...
OUT="${ORBLIT_BUILD_DIR:-$PWD/build}"
case "$OUT" in
  /*) ;;
  *) OUT="$PWD/$OUT" ;;
esac
ORBLIT_FILAMENT_SDK="$FILAMENT" \
ORBLIT_GENERATED_SET="$GENERATED_SET" \
ORBLIT_FILAMENT_BACKEND=webgpu \
ORBLIT_BUILD_DIR="$OUT" \
  bash ../headless/build.sh

if [ "${1:-}" = "test" ]; then
  "$OUT/orblit_headless" "${2:-$OUT/webgpu.png}" webgpu
fi
