...
#
#   build.sh          builds ./build/orblit_host
#   build.sh run      and runs it
#   build.sh test     builds and runs orblit_pad's tests alone, which need no
#                     Filament, no SDL and no window and take a second
#
#   ORBLIT_FILAMENT_SDK   a Filament release (the directory holding include/
#                        and lib/). Defaults to the one darwin/setup.sh
#                        staged; required on Linux.
#   ORBLIT_FILAMENT_ARCH  which slice of lib/ to link. Defaults to arm64 on
#                        macOS and `uname -m` on Linux.
#   ORBLIT_BUILD_DIR      where the objects and the program go (default: build)
#   ORBLIT_SDL_CFLAGS     )  how to find SDL3, when pkg-config cannot. Both
#   ORBLIT_SDL_LIBS       )  are asked of pkg-config otherwise.
#
# The materials must have been compiled for the backend this will run:
# darwin/setup.sh does Metal by default, and ORBLIT_MATC_BACKENDS names others
...
# guarding against is package:orblit_input and this drifting apart quietly.
mkdir -p "$OUT"
clang -std=c99 -O1 -Wall -Wextra -Werror -pedantic -I . \
  orblit_pad.c orblit_pad_test.c -lm -o "$OUT/orblit_pad_test"
if [ "${1:-}" = "test" ]; then
  exec "$OUT/orblit_pad_test"
fi
"$OUT/orblit_pad_test"
...
case "$SYSTEM" in
  Darwin)
    ...
    ARCH="${ORBLIT_FILAMENT_ARCH:-arm64}"
    ...
    ;;
  Linux)
    SDK="${ORBLIT_FILAMENT_SDK:?ORBLIT_FILAMENT_SDK must point at a Linux Filament release}"
    ARCH="${ORBLIT_FILAMENT_ARCH:-$(uname -m)}"
...
if [ ! -f "$SRC/generated/lit_opaque_material.h" ]; then
  ...
  echo "run packages/orblit_filament/darwin/setup.sh first."
  exit 1
fi

SDL_CFLAGS="${ORBLIT_SDL_CFLAGS:-$(pkg-config --cflags sdl3)}"
SDL_LIBS="${ORBLIT_SDL_LIBS:-$(pkg-config --libs sdl3)}"
...
# than forgotten. ORBLIT_PLATFORM_PORTABLE is the set of files a Linux,
# Android, Windows or console build takes -- OrblitPlatformApple.mm,
# OrblitSurfaceApple.mm and the Objective-C wrapper are not compiled at all.
objects=()
for source in "$SRC"/*.cpp; do
  name="$(basename "$source" .cpp)"
  clang++ -std=c++17 -O2 -DORBLIT_PLATFORM_PORTABLE "${STDLIB[@]+"${STDLIB[@]}"}" \
...
# C99 and pedantic, so anything C++ that creeps into orblit_renderer.h or
# orblit_pad.h is an error here rather than a surprise in a console
# toolchain's C compiler later. SDL's own headers are C.
# shellcheck disable=SC2086 -- pkg-config's flags are meant to split.
for source in orblit_host orblit_host_sdl orblit_pad; do
...
clang++ "$OUT/orblit_host.o" "$OUT/orblit_host_sdl.o" "$OUT/orblit_pad.o" \
  "${objects[@]}" "${STDLIB[@]+"${STDLIB[@]}"}" \
  "${GROUP_OPEN[@]+"${GROUP_OPEN[@]}"}" "${archives[@]}" \
  "${GROUP_CLOSE[@]+"${GROUP_CLOSE[@]}"}" \
  $SDL_LIBS "${PLATFORM_LINK[@]}" -o "$OUT/orblit_host"

echo "built $OUT/orblit_host"
