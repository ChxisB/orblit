#ifndef ORBLIT_HOST_PLATFORM_H
#define ORBLIT_HOST_PLATFORM_H
 ...
 *
 * The seam is drawn exactly where OrblitSurface.h draws the renderer's own:
 ...
 *
 * orblit_host_sdl.c answers all of this with SDL3 for macOS, Linux and
 ...
 * nothing below trusts them and orblit_host_size is asked instead. Non-zero on
 ...
 *
 * The same contract as orblit::now() in the renderer's own platform layer, and
 * the number handed to orblit_renderer_draw. A host owns the clock — that is
...
/* Where every pad stood at the last orblit_host_pump. Never NULL: with nothing
 * plugged in, every slot reads as an absent pad holding nothing. */
const orblit_pad_frame *orblit_host_pads(void);

#endif /* ORBLIT_HOST_PLATFORM_H */
