...
`../orblit-filament`. Where it is not, it says so.
...
C++ in `darwin/.../orblit_filament_native/*.cpp` — does not change for a new
...
`orblit_host_sdl.c` — are the only ones that know what machine they are on.
...
puts the X11 `Window` cast that `orblit_host_sdl.c` feeds it, and that path is
...
| Swapchain acquire/present semantics and the frame pacing they imply | `orblit_host.c` and `orblit_pad.c` — 580 lines that are the same everywhere |
| The pad API, the clock, the window: `orblit_host_platform.h`'s eight calls | `orblit_pad.h`'s vocabulary, shaping, slots and hot-plug |
| The file system, the memory budget, the shader precompile format | The materials: `ORBLIT_MATC_BACKENDS=vulkan darwin/setup.sh` compiles SPIR-V today |
| Certification: what a title must do on suspend, resume, and controller loss | `orblit_renderer_detach_surface` / `attach_surface`, written for exactly that lifecycle on Android |
...
3. Rewrite `orblit_host_sdl.c` against the SDK's window, clock and pad. The
   eight calls are the whole list; `orblit_pad_raw` is the whole of what a pad
   back end fills in.
4. `ORBLIT_MATC_BACKENDS=vulkan darwin/setup.sh`, and run the headless host
...
clean, and Orblit's headless scene **draws through Dawn onto Metal** —
...
WebGPU driver, Tint, the WGSL Orblit's materials compile to, the descriptor
...
are empty), so `orblit_stats.gpu_milliseconds` reads 0.00 on WebGPU and no
...
| `WebGPUPlatformXbox::createSurface` — 95 lines by the shape of the Windows one, if the console's swapchain is HWND-shaped | The materials: `ORBLIT_MATC_BACKENDS=webgpu` compiles 25 of 26 today |
| Whether the console permits Dawn's runtime shader compilation at all | `orblit_host.c`, `orblit_pad.c`, the renderer, the C ABI |
| The pad, the window, the clock: the same eight calls | `orblit_pad.h`'s vocabulary — Xbox's pad is the one SDL's positional names were written for |
...
*Filament and Tint and Orblit's materials*, and deliberately makes no claim
about D3D12 or about the console.

## Game logic, without Dart

Dart is how Orblit is written and it is not going to a console: there is no
...
- **QuickJS.** `orblit-script` puts game logic in TypeScript on QuickJS,
...
- **Linked C++.** `orblit_native`'s `include/orblit_script.h` already defines
...
Dart: `packages/orblit_core`, `orblit_mesh`, `orblit_light` and the rest build
the arrays that `orblit_renderer_apply_*` takes, and the C ABI is deliberately
...
`orblit_host.c` is an existence proof of the second at its smallest: 324 lines
...
| Front end | this host, `orblit_host_sdl.c` rewritten | the same |
| Game logic | QuickJS or linked C++; the scene layer is the real gap | the same |
| Measured here | the Vulkan path runs on lavapipe in a container | Dawn draws Orblit's scene on Metal; 25 of 26 materials compile for WebGPU |
