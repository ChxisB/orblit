/* Shaping, slots and edges: package:orblit_input's decisions, in C.
 *
 * Nothing here talks to a platform. A back end (orblit_host_sdl.c on a desktop,
 * something under an NDA on a console) reports what it found this poll as
 * orblit_pad_raw, and this file turns that into the orblit_pad_frame a game
 * reads. The split is the whole point: a console port replaces the back end
 * and this file is not touched.
 *
 * The Dart original is packages/orblit_input/lib/src/shaping.dart and
...
/* orblit_input's Shaping() default and Shaping.triggers, field for field. */
const orblit_shaping orblit_shaping_sticks = {0.15f, 0.95f, 1.0f};
const orblit_shaping orblit_shaping_triggers = {0.06f, 0.98f, 1.0f};

/* `magnitude`, nought to one, with the zone taken out and the rest rescaled.
 * Shaping._shape. */
static float shape_magnitude(const orblit_shaping *shaping, float magnitude) {
...
int orblit_pad_is_down(const orblit_pad_state *pad, orblit_pad_button button) {
  if (pad == NULL || button < 0 || button >= ORBLIT_PAD_BUTTON_COUNT) return 0;
...
void orblit_pad_dpad(const orblit_pad_state *pad, float *out_x, float *out_y) {
  const float x = (float)(orblit_pad_is_down(pad, ORBLIT_PAD_DPAD_RIGHT) -
                          orblit_pad_is_down(pad, ORBLIT_PAD_DPAD_LEFT));
  const float y = (float)(orblit_pad_is_down(pad, ORBLIT_PAD_DPAD_UP) -
                          orblit_pad_is_down(pad, ORBLIT_PAD_DPAD_DOWN));
...
static const orblit_pad_state kAbsent;

const orblit_pad_state *orblit_pad_first(const orblit_pad_frame *frame) {
  if (frame != NULL) {
    for (int i = 0; i < ORBLIT_PAD_SLOTS; i++) {
 ...
 * happens to be free. orblit_input keeps the same fact in a separate
 ...
 */
typedef struct {
  char token[sizeof(((orblit_pad_raw *)0)->token)];
  char name[64];
...
struct orblit_pads {
  orblit_slot slots[ORBLIT_PAD_SLOTS];
};

orblit_pads *orblit_pads_create(void) {
  return (orblit_pads *)calloc(1, sizeof(orblit_pads));
}

void orblit_pads_destroy(orblit_pads *pads) { free(pads); }

/* Which slot this token already owns, or -1. */
static int slot_of(const orblit_pads *pads, const char *token) {
  for (int i = 0; i < ORBLIT_PAD_SLOTS; i++) {
...
void orblit_pads_poll(orblit_pads *pads, const orblit_pad_raw *raw, int count,
  ...
  for (int i = 0; i < ORBLIT_PAD_SLOTS; i++) pads->slots[i].present = 0;
  ...
  for (int i = 0; i < consider; i++) {
    ...
    for (int s = 0; s < ORBLIT_PAD_SLOTS && at < 0; s++) {
  ...
  for (int i = 0; i < consider; i++) {
    ...
    if (at < 0) continue;
    orblit_slot *slot = &pads->slots[at];
    orblit_pad_state *state = &frame->pads[at];
    const orblit_pad_raw *from = &raw[i];
    ...
    const uint32_t mask = (1u << ORBLIT_PAD_BUTTON_COUNT) - 1u;
    ...
    for (int side = 0; side < 2; side++) {
      ...
      orblit_shape_stick(&orblit_shaping_sticks, x, y, &state->stick_x[side],
                        &state->stick_y[side]);
      state->trigger[side] =
          orblit_shape_scalar(&orblit_shaping_triggers, from->trigger[side]);
    }
     ...
     * immediate — and measured on the *shaped* value, as orblit_input measures
     ...
     * this adds nothing. */
    if (state->trigger[ORBLIT_PAD_LEFT] > 0.5f) {
      state->down |= 1u << ORBLIT_PAD_LEFT_TRIGGER;
    }
    if (state->trigger[ORBLIT_PAD_RIGHT] > 0.5f) {
      state->down |= 1u << ORBLIT_PAD_RIGHT_TRIGGER;
    }
