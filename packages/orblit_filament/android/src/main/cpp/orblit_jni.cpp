// The JNI bridge: dev.orblit.filament.OrblitNative (Kotlin) <-> orblit_renderer.h
// (C ABI) <-> orblit::Renderer (the portable core).
//
// Every function here does what OrblitRendererC.cpp's own comment says of
...
// ORBLIT_ERROR_* to report, translated to a Kotlin Int a Kotlin `orblit_result`
// alias reads the same way.
//
// Naming: dev.orblit.filament.OrblitNative, an `object`, so every external fun
...
// OrblitSurface (inside the core) only borrows the pointer it is given
// (orblit_surface_desc.window), so releasing it is this layer's job, exactly
// as it is SpikeRenderer's in the android-spike. Not owned: the orblit_renderer
// itself is destroyed through the ABI, which also destroys the OrblitSurface
...
struct OrblitHandle {
  orblit_renderer *renderer = nullptr;
  ANativeWindow *window = nullptr;
};

OrblitHandle *fromHandle(jlong handle) {
  return reinterpret_cast<OrblitHandle *>(handle);
}
...
// empty", per orblit_view.dart's own comment on non-nullable pointers) borrows
...
Java_dev_orblit_filament_OrblitNative_nativeCreate(JNIEnv *env, jclass, jint backendOrdinal,
  ...
  orblit_surface_desc desc{};
  desc.kind = window != nullptr ? ORBLIT_SURFACE_WINDOW : ORBLIT_SURFACE_HEADLESS;
  desc.window = window;

  orblit_renderer *renderer = orblit_renderer_create(
      static_cast<OrblitBackend>(backendOrdinal), &desc, uint32_t(width), uint32_t(height));
  if (renderer == nullptr) {
    if (window != nullptr) ANativeWindow_release(window);
    LOGE("nativeCreate: orblit_renderer_create refused (backend=%d)", int(backendOrdinal));
    return 0;
  }

  auto *handle = new OrblitHandle{renderer, window};
  return reinterpret_cast<jlong>(handle);
}

JNIEXPORT void JNICALL
Java_dev_orblit_filament_OrblitNative_nativeDestroy(JNIEnv *, jclass, jlong handleValue) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return;
  orblit_renderer_destroy(handle->renderer);
  // Whatever the last attached window was, released only now: the core has
  // already destroyed its swap chain and flushed (inside orblit_renderer_destroy
...
Java_dev_orblit_filament_OrblitNative_nativeBackend(JNIEnv *, jclass, jlong handleValue) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_BACKEND_DEFAULT;
  return orblit_renderer_backend(handle->renderer);
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeResize(JNIEnv *, jclass, jlong handleValue, jint width,
        jint height) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  return orblit_renderer_resize(handle->renderer, uint32_t(width), uint32_t(height));
}
...
Java_dev_orblit_filament_OrblitNative_nativeAttachSurface(JNIEnv *env, jclass, jlong handleValue,
        jobject surface, jint width, jint height) {
  auto *handle = fromHandle(handleValue);
  ...
  const int result =
      orblit_renderer_attach_surface(handle->renderer, &desc, uint32_t(width), uint32_t(height));
  ...
  // its comment in OrblitRendererCore.cpp. Only now is it safe to let the
  // previous window go.
  ANativeWindow *previous = handle->window;
  handle->window = window;
  if (previous != nullptr) ANativeWindow_release(previous);

  return result == ORBLIT_OK ? JNI_TRUE : JNI_FALSE;
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeDetachSurface(JNIEnv *, jclass, jlong handleValue) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  const int result = orblit_renderer_detach_surface(handle->renderer);
...
JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeDraw(JNIEnv *, jclass, jlong handleValue,
        jdouble seconds) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  return orblit_renderer_draw(handle->renderer, seconds);
}

// ---- The scene ---------------------------------------------------------
//
// One JNI function per orblit_renderer_* scene call, in the ABI's own order,
// each a direct translation of its arguments -- the shape OrblitRendererC.cpp
// itself already is one layer down. Counts are derived from array lengths
// the same way OrblitFilamentPlugin.swift's Scene.init does (transforms.count
// / 16, and so on), not sent separately, since that is what the wire format
// already is.

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplyObjects(JNIEnv *env, jclass, jlong handleValue,
  ...
  return orblit_renderer_apply_objects(handle->renderer, count, k.ptr(), t.ptr(), t.count(),
...
Java_dev_orblit_filament_OrblitNative_nativeSetBatching(JNIEnv *, jclass, jlong handleValue,
  ...
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  return orblit_renderer_set_batching(handle->renderer, enabled == JNI_TRUE ? 1 : 0);
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetDepthPrepass(JNIEnv *, jclass, jlong handleValue,
        jboolean enabled) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  return orblit_renderer_set_depth_prepass(handle->renderer, enabled == JNI_TRUE ? 1 : 0);
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplyMaterials(JNIEnv *env, jclass, jlong handleValue,
  ...
  Ints vid(env, videos);
  return orblit_renderer_apply_materials(handle->renderer, k.count(), k.ptr(), f.ptr(), pr.ptr(),
      pr.count(), mp.ptr(), mp.count(), tp.ptr(), srgb.ptr(), tp.count(), vid.ptr());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetPipeline(JNIEnv *env, jclass, jlong handleValue,
        jfloatArray params) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Floats pr(env, params);
  return orblit_renderer_set_pipeline(handle->renderer, pr.ptr(), pr.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplyVideos(JNIEnv *env, jclass, jlong handleValue,
  ...
  Floats pr(env, params);
  Strings p(env, paths);
  return orblit_renderer_apply_videos(
      handle->renderer, k.count(), k.ptr(), f.ptr(), pr.ptr(), pr.count(), p.ptr(), p.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplyLights(JNIEnv *env, jclass, jlong handleValue,
  ...
  Ints f(env, flags);
  Floats pr(env, params);
  return orblit_renderer_apply_lights(
      handle->renderer, k.count(), k.ptr(), kd.ptr(), f.ptr(), pr.ptr(), pr.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplyDecals(JNIEnv *env, jclass, jlong handleValue,
  ...
  Ints im(env, images);
  Strings p(env, paths);
  return orblit_renderer_apply_decals(
      handle->renderer, im.count(), pr.ptr(), pr.count(), im.ptr(), p.ptr(), p.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetFog(JNIEnv *env, jclass, jlong handleValue,
        jboolean enabled, jfloatArray params) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Floats pr(env, params);
  return orblit_renderer_set_fog(handle->renderer, enabled == JNI_TRUE ? 1 : 0, pr.ptr(), pr.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetPostProcess(JNIEnv *env, jclass, jlong handleValue,
        jfloatArray params) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Floats pr(env, params);
  return orblit_renderer_set_post_process(handle->renderer, pr.ptr(), pr.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplyProbes(JNIEnv *env, jclass, jlong handleValue,
        jlongArray keys, jfloatArray params) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Longs k(env, keys);
  Floats pr(env, params);
  return orblit_renderer_apply_probes(handle->renderer, k.count(), k.ptr(), pr.ptr(), pr.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplyField(JNIEnv *env, jclass, jlong handleValue,
        jfloatArray params, jstring from) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Floats pr(env, params);
  const std::string source = toStdString(env, from);
  return orblit_renderer_apply_field(handle->renderer, pr.ptr(), pr.count(), source.c_str());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetEnvironment(JNIEnv *env, jclass, jlong handleValue,
  ...
  Floats pr(env, params);
  return orblit_renderer_set_environment(handle->renderer, r.c_str(), s.c_str(), pr.ptr(), pr.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetRenderGraph(JNIEnv *env, jclass, jlong handleValue,
  ...
  // Strides are the ABI's own (orblit_renderer_stride), matching
  // Scene.passStride/targetStride on the Swift side and OrblitRenderGraph on
  // the Dart one -- see orblit_renderer.h's ORBLIT_STRIDE_PASS/TARGET.
  const uint32_t passStride = orblit_renderer_stride(ORBLIT_STRIDE_PASS);
  const uint32_t targetStride = orblit_renderer_stride(ORBLIT_STRIDE_TARGET);
  ...
  return orblit_renderer_set_render_graph(handle->renderer, passCount, ps.ptr(), ps.count(),
      targetCount, ts.ptr(), ts.count(), nm.ptr(), nm.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetGodRays(JNIEnv *env, jclass, jlong handleValue,
        jfloatArray godRays, jfloatArray distortions) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Floats gr(env, godRays);
  Floats di(env, distortions);
  return orblit_renderer_set_god_rays(handle->renderer, gr.ptr(), gr.count(), di.ptr(), di.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplyPopulations(JNIEnv *env, jclass, jlong handleValue,
  ...
  return orblit_renderer_apply_populations(handle->renderer, k.count(), k.ptr(), c.ptr(), m.ptr(),
      f.ptr(), rev.ptr(), rg.ptr(), bd.ptr(), bd.count(), p.ptr(), p.count(), ch.ptr(), ch.count(),
      tr.ptr(), tr.count(), co.ptr(), co.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeApplySplats(JNIEnv *env, jclass, jlong handleValue,
  ...
  Bytes by(env, data);
  return orblit_renderer_apply_splats(handle->renderer, k.count(), k.ptr(), f.ptr(), rev.ptr(),
...
Java_dev_orblit_filament_OrblitNative_nativeSetSky(JNIEnv *env, jclass, jlong handleValue,
  ...
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Floats pr(env, params);
  return orblit_renderer_set_sky(handle->renderer, enabled == JNI_TRUE ? 1 : 0, pr.ptr(), pr.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetPrecipitation(JNIEnv *env, jclass, jlong handleValue,
        jboolean enabled, jfloatArray params) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Floats pr(env, params);
  return orblit_renderer_set_precipitation(
      handle->renderer, enabled == JNI_TRUE ? 1 : 0, pr.ptr(), pr.count());
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetSkyColour(JNIEnv *env, jclass, jlong handleValue,
        jfloatArray colour, jfloat ambient, jboolean showBody) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Floats c(env, colour);
  if (c.count() < 3) return ORBLIT_ERROR_NULL;
  return orblit_renderer_set_sky_colour(
      handle->renderer, c.ptr(), ambient, showBody == JNI_TRUE ? 1 : 0);
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetCamera(JNIEnv *env, jclass, jlong handleValue,
  ...
  Floats t(env, target);
  if (p.count() < 3 || t.count() < 3) return ORBLIT_ERROR_NULL;
  return orblit_renderer_set_camera(handle->renderer, p.ptr(), t.ptr(), fieldOfView,
      orthographic == JNI_TRUE ? 1 : 0, viewHeight, at);
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetExposure(JNIEnv *, jclass, jlong handleValue,
        jfloat aperture, jfloat shutter, jfloat sensitivity) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  return orblit_renderer_set_exposure(handle->renderer, aperture, shutter, sensitivity);
}

JNIEXPORT jint JNICALL
Java_dev_orblit_filament_OrblitNative_nativeSetOutline(JNIEnv *env, jclass, jlong handleValue,
        jlongArray keys, jfloatArray params) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return ORBLIT_ERROR_NULL;
  Longs k(env, keys);
  Floats pr(env, params);
  return orblit_renderer_set_outline(handle->renderer, k.ptr(), k.count(), pr.ptr(), pr.count());
}

// ---- What it drew, and what it cost ------------------------------------

JNIEXPORT jdouble JNICALL
Java_dev_orblit_filament_OrblitNative_nativeGpuMilliseconds(JNIEnv *, jclass, jlong handleValue) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return 0.0;
  orblit_stats stats{};
  if (orblit_renderer_stats(handle->renderer, &stats) != ORBLIT_OK) return 0.0;
  return stats.gpu_milliseconds;
}

// [batchedObjects, batchGroups].
JNIEXPORT jintArray JNICALL
Java_dev_orblit_filament_OrblitNative_nativeBatching(JNIEnv *env, jclass, jlong handleValue) {
  jintArray out = env->NewIntArray(2);
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return out;
  orblit_stats stats{};
  if (orblit_renderer_stats(handle->renderer, &stats) != ORBLIT_OK) return out;
...
// which OrblitView.capture() (orblit_view.dart) already knows how to read.
JNIEXPORT jdoubleArray JNICALL
Java_dev_orblit_filament_OrblitNative_nativePassTimings(JNIEnv *env, jclass, jlong handleValue) {
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return env->NewDoubleArray(0);
  const uint32_t count = orblit_renderer_pass_timings(handle->renderer, nullptr, nullptr, 0);
  std::vector<double> milliseconds(count);
  std::vector<int32_t> drawn(count);
  orblit_renderer_pass_timings(handle->renderer, milliseconds.data(), drawn.data(), count);
...
Java_dev_orblit_filament_OrblitNative_nativeNotes(JNIEnv *env, jclass, jlong handleValue) {
  jclass stringClass = env->FindClass("java/lang/String");
  auto *handle = fromHandle(handleValue);
  if (handle == nullptr) return env->NewObjectArray(0, stringClass, nullptr);
  const uint32_t count = orblit_renderer_notes(handle->renderer);
  jobjectArray out = env->NewObjectArray(jsize(count) * 2, stringClass, nullptr);
  for (uint32_t i = 0; i < count; i++) {
    const char *about = nullptr;
    const char *saying = nullptr;
    if (orblit_renderer_note(handle->renderer, i, &about, &saying) != ORBLIT_OK) continue;
