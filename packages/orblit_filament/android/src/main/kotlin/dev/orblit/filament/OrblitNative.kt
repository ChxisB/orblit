...
/**
 * The JNI surface of liborblit_filament_native.so, and nothing else.
 *
 * A Kotlin `object`, so every declaration below is an instance method on the
 * singleton and the C++ side (orblit_jni.cpp) receives a `jobject`/`jclass` it
 * ignores -- see that file's own header comment for why this is safe despite
 * looking like a mismatch. `handle` is the C++ OrblitHandle*, carried as a
 ...
 * orblit_renderer.h's own C functions all take the orblit_renderer* they act
 * on rather than there being one instance per renderer.
 *
 * Every `Int` return here is an `orblit_result` (orblit_renderer.h): 0 is
 * ORBLIT_OK, negative is a specific refusal. OrblitFilamentPlugin does not
 ...
 */
internal object OrblitNative {
    init {
        System.loadLibrary("orblit_filament_native")
    }

    // ---- Lifetime -----------------------------------------------------

    /**
     * [backendOrdinal] matches OrblitBackend in orblit_renderer.h exactly: 0
     ...
     * orblit_renderer_attach_surface's own comment for why this exists and
    ...
    // ---- The scene ------------------------------------------------------
    //
    // One per orblit_renderer_apply_*/set_* call, in the ABI's own order.
    ...
    // the C++ side -- see orblit_jni.cpp's Floats/Ints/etc. Never null itself:
    // OrblitScene.kt fills in the same empty-array defaults
    // OrblitFilamentPlugin.swift's Scene.init does, so every call below
