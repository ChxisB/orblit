...
/**
 * A scene as it arrives over the `orblit_filament` channel's `setScene` call,
 ...
 *
 * Mirrors OrblitFilamentPlugin.swift's private `Scene` struct field for
 ...
 * Dart's own `OrblitScene.toMessage()` already builds correctly, the one
 ...
 */
internal class OrblitScene private constructor(private val args: Map<String, Any?>) {
    ...
    // Splats: OrblitSplatMessage.swift's own field names, unchanged.
    ...
    fun applyTo(handle: Long) {
        ...
        OrblitNative.nativeSetRenderGraph(handle, graphPasses, graphTargets, graphTargetNames.toTypedArray())

        OrblitNative.nativeSetBatching(handle, batching)
        OrblitNative.nativeSetDepthPrepass(handle, depthPrepass)
        OrblitNative.nativeSetGodRays(handle, godRayParams, distortionParams)

        OrblitNative.nativeApplyVideos(handle, videoKeys, videoFlags, videoParams, videoPaths.toTypedArray())

        OrblitNative.nativeApplyMaterials(
            handle,
            materialKeys,
            materialFlags,
            materialParams,
        ...
        OrblitNative.nativeApplyObjects(
            handle,
            keys,
            transforms,
            colours,
        ...
        if (populationKeys.isNotEmpty()) {
            OrblitNative.nativeApplyPopulations(
                handle,
                populationKeys,
                populationCounts,
                populationMeshes,
        ...
        OrblitNative.nativeApplySplats(
            handle,
            splatKeys,
            splatFlags,
            splatRevisions,
        ...
        OrblitNative.nativeApplyDecals(handle, decalParams, decalImages, decalPaths.toTypedArray())

        OrblitNative.nativeApplyProbes(handle, probeKeys, probeParams)

        if (fieldParams.isNotEmpty()) OrblitNative.nativeApplyField(handle, fieldParams, fieldFrom)

        OrblitNative.nativeSetSkyColour(handle, skyColour, ambient, showBody)
        OrblitNative.nativeSetFog(handle, fogEnabled, fogParams)
        if (postParams.isNotEmpty()) OrblitNative.nativeSetPostProcess(handle, postParams)
        if (pipelineParams.isNotEmpty()) OrblitNative.nativeSetPipeline(handle, pipelineParams)
        OrblitNative.nativeSetPrecipitation(handle, precipitationEnabled, precipitationParams)
        OrblitNative.nativeSetSky(handle, skyEnabled, skyParams)
        OrblitNative.nativeSetCamera(
            handle, cameraPosition, cameraTarget, fieldOfView, orthographic, viewHeight, at)
        OrblitNative.nativeSetExposure(handle, aperture, shutterSpeed, sensitivity)

        OrblitNative.nativeSetOutline(handle, outlineKeys, outlineParams)
    }
    ...
    companion object {
         ...
         */
        fun from(args: Map<String, Any?>): OrblitScene? {
...
// field in OrblitFilamentPlugin.swift's Scene also has.
