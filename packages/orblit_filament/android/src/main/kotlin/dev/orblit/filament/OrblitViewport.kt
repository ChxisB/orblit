 ...
 * while speaking the full `orblit_filament` protocol instead of a hardcoded
 * cube.
 *
 * ## Threading
 *
 ...
 * ## Frame timing
 *
 * `orblit_renderer_draw`'s `seconds` only drives the placeholder cube's idle
...
internal class OrblitViewport(
    ...
    fun start(width: Int, height: Int): Long {
        ...
        // the case where it is not ready until a callback says so. ORBLIT_
        // BACKEND_DEFAULT (0): the platform's own choice (orblit::
        // backendCandidates prefers Vulkan then OpenGL ES off Apple), or
        // whatever ORBLIT_BACKEND names, set as a real process environment
        ...
        handle = OrblitNative.nativeCreate(ORBLIT_BACKEND_DEFAULT, initial, width, height)
        if (handle == 0L) {
            p.setCallback(null)
            p.release()
            producer = null
    ...
    fun resize(width: Int, height: Int) {
        ...
        p.setSize(width, height)
        OrblitNative.nativeResize(handle, width, height)
     ...
     */
    fun applyScene(scene: OrblitScene, answered: (Map<String, String>) -> Unit) {
        if (handle == 0L) {
            answered(emptyMap())
            return
        }
        scene.applyTo(handle)
        val notes = OrblitNative.nativeNotes(handle)
    ...
    fun stats(): Map<String, Any?> {
        ...
        return mapOf(
            "gpuMilliseconds" to OrblitNative.nativeGpuMilliseconds(handle),
            "passTimings" to OrblitNative.nativePassTimings(handle),
            "batching" to OrblitNative.nativeBatching(handle),
        )
    }
    ...
    fun dispose() {
        ...
        if (handle != 0L) {
            OrblitNative.nativeDestroy(handle)
            handle = 0L
        }
    ...
    override fun onSurfaceCleanup() {
        ...
        if (handle != 0L) OrblitNative.nativeDetachSurface(handle)
        attachedSurface = null
    }
    ...
    override fun doFrame(frameTimeNanos: Long) {
        ...
        if (OrblitNative.nativeDraw(handle, seconds) == 0) {
    ...
    private fun attach(reason: String): Boolean {
        ...
        val ok = OrblitNative.nativeAttachSurface(handle, surface, p.width, p.height)
    ...
    private companion object {
        const val TAG = "OrblitFilament"

        // Matches OrblitBackend in orblit_renderer.h.
        const val ORBLIT_BACKEND_DEFAULT = 0
    }
}
