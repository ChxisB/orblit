...
fail() { echo "orblit_filament/android/setup.sh: $*" >&2; exit 1; }
GENERATED_ROOT="$DARWIN_NATIVE/generated"
MATERIAL_SET="${ORBLIT_GENERATED_SET:-android-release}"
case "$MATERIAL_SET" in
  ''|.|..|*/*)
    fail "ORBLIT_GENERATED_SET must be a simple directory name"
    ;;
esac
...
if [ -d "$FILAMENT_ANDROID_DIR/include" ]; then
  ...
  echo "orblit_filament/android: third_party/filament -> $FILAMENT_ANDROID_DIR"
elif [ -d "$FILAMENT/include" ]; then
  echo "orblit_filament/android: Filament $FILAMENT_VERSION (android) already present"
else
  echo "orblit_filament/android: fetching Filament $FILAMENT_VERSION (android)"
  ...
  curl -fsSL -o /tmp/orblit_ltc_android.js "$LTC_FROM"
  {
    ...
    slice() {
      ...
      awk -v want="$1" '
        ...
        { print line }
      ' /tmp/orblit_ltc_android.js
      echo "};"
    }
    slice g_ltc_1 kLtcMatrix
    slice g_ltc_2 kLtcFresnel
  } > "$LTC_HEADER"
  rm -f /tmp/orblit_ltc_android.js
fi
...
ORBLIT_MATC_BACKENDS="${ORBLIT_MATC_BACKENDS:-opengl vulkan}"
MATC_API=""
...
MATC_PROFILE="${ORBLIT_MATC_PROFILE:-mobile}"
...
compile() {
  ...
  echo "orblit_filament/android: compiling $name"
  local input="$source"
  if [ -n "$blend" ]; then
    input="/tmp/orblit_android_src_$name.mat"
    sed "s/^\( *blending *: *\)[a-z]*,/\1$blend,/" "$source" > "$input"
  fi
  # shellcheck disable=SC2086 -- the flags are ours and are meant to split.
  "$MATC" $MATC_FLAGS -o "/tmp/orblit_android_$name.filamat" "$input"
  (cd /tmp && xxd -i "orblit_android_$name.filamat") \
    | sed "s/orblit_android_${name}_filamat/k${name}Material/g" > "$header"
  rm -f "/tmp/orblit_android_$name.filamat" "/tmp/orblit_android_src_$name.mat"
}
...
echo "orblit_filament/android: setup complete"
