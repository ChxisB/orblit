// The web's `orblit_filament` plugin: the same channel, the same methods, the
// same wire shapes as OrblitFilamentPlugin.swift and OrblitFilamentPlugin.kt, so
// `lib/src/orblit_view.dart` and every other Dart caller work unchanged.
//
// One [OrblitWebViewport] per `create`d id, keyed by that id — `_viewports`
...
// `platforms: web: pluginClass: OrblitFilamentWeb` makes the generated
...
import 'orblit_viewport_web.dart';

/// The web implementation of the `orblit_filament` plugin.
class OrblitFilamentWeb {
  OrblitFilamentWeb._();

  static final OrblitFilamentWeb _instance = OrblitFilamentWeb._();
  ...
  static void registerWith(Registrar registrar) {
    ...
    ui_web.platformViewRegistry.registerViewFactory(orblitWebViewType, (
      int viewId, {
      Object? params,
    }) {
      ...
      if (viewport == null) {
        ...
        return OrblitWebViewport(-1).createCanvas();
      }
