...
// `orblit_renderer.js` + `orblit_renderer.wasm` with `-sMODULARIZE=1
// -sEXPORT_NAME=OrblitRendererModule`, exporting every `orblit_renderer_*`
// function orblit_renderer.h names plus `_malloc`, `_free` and
// `_orblit_web_create_on_canvas`. This file is the Dart half of what
...
//
// Nothing here knows what a scene is; see orblit_scene_web.dart for that.
library;
...
/// The name `orblit_renderer.js` defines on `globalThis`, from build.sh's
/// `-sEXPORT_NAME=OrblitRendererModule`.
const String _factoryName = 'OrblitRendererModule';

/// Whether `orblit_renderer.js` has been loaded into the page at all.
///
...
/// a function".
bool get orblitRendererScriptLoaded => globalContext.has(_factoryName);

/// What `OrblitRendererModule({canvas})` resolves to.
///
...
extension type OrblitModule._(JSObject _) implements JSObject {
...
/// module instance per canvas, so two `OrblitView`s on a page are two
/// independent engines — see orblit_filament_web.dart for what that costs.
Future<OrblitModule> loadOrblitModule(JSObject canvas) async {
  if (!orblitRendererScriptLoaded) {
    throw StateError(
      'orblit_renderer.js is not loaded. Add '
      '<script src="orblit_renderer.js"></script> to web/index.html, and copy '
      'orblit_renderer.js and orblit_renderer.wasm into the app\'s web/ '
      'directory (see packages/orblit_filament/native/web/README.md).',
    );
  }
  ...
  return OrblitModule._(await promise.toDart);
}
...
class OrblitHeap {
  OrblitHeap(this._module);

  final OrblitModule _module;
...
/// ABI takes once arrays have been copied in by [OrblitHeap]; `'string'` is
...
int orblitCall(OrblitModule module, String name, List<Object?> args) {
  ...
  for (final arg in args) {
    switch (arg) {
      ...
      case null:
        ...
        values.add(0.toJS);
      default:
        throw ArgumentError('orblitCall($name): cannot marshal $arg');
    }
  }
