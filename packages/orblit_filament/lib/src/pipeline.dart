...
enum OrblitShadowKind {
  ...
  const OrblitShadowKind(this.label);
...
/// frame whatever Orblit does. It would have to be Filament's change.
class OrblitShadows {
  OrblitShadows({
    this.enabled = true,
    this.kind = OrblitShadowKind.sharp,
    this.mapSize = 1024,
    this.cascades = 2,
    this.distance = 0,
    this.lambda = 0.5,
    ...
    this.contactSteps = 8,
    OrblitVarianceShadows? variance,
  }) : variance = variance ?? OrblitVarianceShadows();
  ...
  /// How wide the penumbra is, for [OrblitShadowKind.soft] and
  /// [OrblitShadowKind.area] — the two that work one out rather than filtering
  /// a fixed width.
  ///
  ...
  /// For [OrblitShadowKind.area]: how much faster the penumbra widens with the
  ...
  /// The settings only [OrblitShadowKind.variance] reads.
  final OrblitVarianceShadows variance;
}
...
class OrblitLighting {
  OrblitLighting({this.clusterNear = 5, this.clusterFar = 100});
...
class OrblitDisplay {
  const OrblitDisplay({
    ...
    this.detail = OrblitDetail.medium,
  });

  /// A Steam Deck's own screen.
  static const OrblitDisplay handheld = OrblitDisplay(width: 1280, height: 800);
  ...
  OrblitDisplay copyWith({int? width, int? height, OrblitDetail? detail}) =>
      OrblitDisplay(
  ...
  @override
  String toString() => 'OrblitDisplay(${width}x$height, ${detail.label})';
}
...
/// describes. [OrblitDetail] gives four settings of those dials that have
...
class OrblitPipeline {
  OrblitPipeline({
    OrblitShadows? shadows,
    OrblitResolution? resolution,
    OrblitLighting? lighting,
    this.samples = 1,
    this.precise = false,
    this.culling = true,
  ...
  factory OrblitPipeline.at(OrblitDetail detail) {
    switch (detail) {
      case OrblitDetail.low:
        return OrblitPipeline(
          shadows: OrblitShadows(mapSize: 512, cascades: 1, distance: 40),
          resolution: OrblitResolution(adaptive: true, minScale: 0.5),
        );
      case OrblitDetail.medium:
        return OrblitPipeline(
          shadows: OrblitShadows(mapSize: 1024, cascades: 2, distance: 80),
        );
      case OrblitDetail.high:
        return OrblitPipeline(
  ...
  /// four [OrblitDetail] has. It is the display's half of the question only.
  factory OrblitPipeline.forDisplay(OrblitDisplay display) {
    final pipeline = OrblitPipeline.at(display.detail);
  ...
  ///
  /// The renderer names every one of these offsets in `OrblitShadows.h`, and
