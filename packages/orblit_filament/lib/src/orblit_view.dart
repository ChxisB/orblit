...
class OrblitView extends StatefulWidget {
  const OrblitView({
    super.key,
    this.scene,
    this.onSceneNotes,
  ...
  static Future<double> gpuMilliseconds(int textureId) async {
    final stats = await _OrblitViewState._channel
  ...
  ///
  /// [passNames] is [OrblitScene.passNames] for the scene that was drawn. The
  ...
  static Future<OrblitFrameCapture> capture(
    int textureId,
    ...
    if (timings is! List) return const OrblitFrameCapture();
    ...
    return OrblitFrameCapture.from(
  ...
  @override
  State<OrblitView> createState() => _OrblitViewState();
}

class _OrblitViewState extends State<OrblitView> {
  static const MethodChannel _channel = MethodChannel('orblit_filament');

  int? _textureId;
  Size? _surfaceSize;
  bool _creating = false;
  Object? _error;
  OrblitScene? _sentScene;

  @override
  void didUpdateWidget(OrblitView oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    // see packages/orblit_filament/android/.
    ...
    // lib/src/web/ and packages/orblit_filament/native/web/.
    // see packages/orblit_filament/android/. Linux is the same again behind
    ...
    // packages/orblit_filament/linux/.
    // see packages/orblit_filament/android/. Windows is the same again behind
    ...
    // Impeller; see packages/orblit_filament/windows/.
    const drawable = {
    ...
    if (!kIsWeb && !drawable.contains(defaultTargetPlatform)) {
      return const _Notice(
        'Orblit renders on macOS, iOS, Android, Linux, Windows and the web '
        'so far.',
      );
    }
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        // lib/src/web/orblit_filament_web.dart finds the viewport it belongs
        // to — a platform view's own id is minted separately by Flutter and
        // never reaches the plugin.
        return kIsWeb
            ? HtmlElementView(viewType: orblitWebViewType, creationParams: id)
