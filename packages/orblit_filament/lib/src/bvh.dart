...
class OrblitBounds {
  const OrblitBounds(this.minimum, this.maximum);

  /// A box around one point.
  factory OrblitBounds.at(Vector3 point) => OrblitBounds(point, point);

  /// A box around everything in [boxes], or null if there are none.
  static OrblitBounds? around(Iterable<OrblitBounds> boxes) {
    OrblitBounds? all;
  ...
  OrblitBounds union(OrblitBounds other) => OrblitBounds(
    Vector3(
  ...
  bool overlaps(OrblitBounds other) =>
...
class OrblitVolume {
  const OrblitVolume(this.key, this.bounds);

  /// Whatever the application calls it — an object's key, an index, an id.
  final int key;

  final OrblitBounds bounds;
}
...
/// them.
class OrblitBvh {
  OrblitBvh._(this._nodes, this._volumes);
  ...
  factory OrblitBvh.of(List<OrblitVolume> volumes, {int leafSize = 4}) {
    final held = [...volumes];
    final nodes = <_Node>[];
    if (held.isNotEmpty) {
      _build(nodes, held, 0, held.length, leafSize.clamp(1, 64));
    }
    return OrblitBvh._(nodes, held);
  }

  final List<_Node> _nodes;
  final List<OrblitVolume> _volumes;
  ...
  OrblitBounds? get bounds => _nodes.isEmpty ? null : _nodes.first.bounds;
  ...
  List<int> inside(OrblitBounds box) {
    final found = <int>[];
  ...
  void refit(OrblitBounds? Function(int key) boundsOf) {
    for (var i = 0; i < _volumes.length; i++) {
      final replacement = boundsOf(_volumes[i].key);
      if (replacement != null) {
        _volumes[i] = OrblitVolume(_volumes[i].key, replacement);
      }
    }
    if (_nodes.isNotEmpty) _refitFrom(0);
  }

  OrblitBounds _refitFrom(int index) {
    final node = _nodes[index];
    if (node.isLeaf) {
  ...
  void _walk(
    ...
    void Function(OrblitVolume) found,
  ) {
  ...
  static bool _inFrustum(OrblitBounds box, List<Vector4> planes) {
  ...
  static int _build(
    List<_Node> nodes,
    List<OrblitVolume> volumes,
    int from,
    int to,
    int leafSize,
  ) {
    var box = volumes[from].bounds;
    var centroids = OrblitBounds.at(volumes[from].bounds.centre);
    for (var i = from + 1; i < to; i++) {
      box = box.union(volumes[i].bounds);
      centroids = centroids.union(OrblitBounds.at(volumes[i].bounds.centre));
    }
    ...
    if (high - low > 1e-9) {
      ...
      final boxes = List<OrblitBounds?>.filled(_buckets, null);
      final scale = _buckets / (high - low);

      int bucketOf(OrblitVolume volume) {
