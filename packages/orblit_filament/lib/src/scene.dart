...
class OrblitScene {
  ...
  /// with the same [OrblitObject.mesh], the same [OrblitObject.material] and the
  /// same flags form a group; a placeholder cube on the default surface also
  /// needs the same [OrblitObject.colour], because on that surface the colour
  /// *is* the material. An object with [OrblitObject.morphWeights] never
  ...
  /// own set of them — give such a model an [OrblitMaterial] and the census
  ...
  /// asking for it. A fix for the underlying Filament bug exists, on Orblit's
