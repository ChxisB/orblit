...
/// an [OrblitEffect.godRays] pass wherever it wants them, and this says how
/// strong they are.
///
...
class OrblitGodRays {
  ...
  /// No shafts at all.
  static const OrblitGodRays off = OrblitGodRays();
...
class OrblitDistortion {
  const OrblitDistortion._({
    required this.kind,
  ...
  factory OrblitDistortion.shockwave({
    ...
    kind: OrblitDistortionKind.shockwave,
    strength: strength,
    chromatic: chromatic,
    centre: centre,
    radius: radius,
  ...
  /// possible to test.
  factory OrblitDistortion.expanding({
    required Vector3 centre,
    required double age,
    double speed = 5,
    double lifetime = 2,
    ...
    return OrblitDistortion.shockwave(
      centre: centre,
  ...
  factory OrblitDistortion.haze({
    ...
    kind: OrblitDistortionKind.haze,
    strength: strength,
    chromatic: chromatic,
    centre: centre,
    halfSize: halfSize,
  ...
  factory OrblitDistortion.lens({
    required double strength,
    double chromatic = 0,
  }) => OrblitDistortion._(
    kind: OrblitDistortionKind.lens,
    strength: strength,
    chromatic: chromatic,
  );

  final OrblitDistortionKind kind;
  ...
  bool get isActive =>
      kind != OrblitDistortionKind.none && strength != 0 && strength.isFinite;
  ...
  void _pack(Float32List into, int at) {
    ...
    into[at + 5] = c.z;
    switch (kind) {
      case OrblitDistortionKind.shockwave:
        into[at + 6] = radius;
        into[at + 7] = thickness;
      case OrblitDistortionKind.haze:
        ...
        into[at + 9] = scale;
        into[at + 10] = rise;
      case OrblitDistortionKind.lens:
      case OrblitDistortionKind.none:
        break;
    }
  }

  /// The ones that move anything, packed end to end — at most [capacity].
  static Float32List packAll(List<OrblitDistortion> distortions) {
