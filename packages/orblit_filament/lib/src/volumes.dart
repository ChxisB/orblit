...
/// of [OrblitFog], [OrblitSky], [OrblitEnvironment], [OrblitPostProcess] or the
/// camera, so a resolved volume is an ordinary scene and the renderer never
/// learns volumes exist.
class OrblitEnvironmentOverrides {
  const OrblitEnvironmentOverrides({
    this.fogDensity,
    this.fogColour,
    this.fogHeight,
  ...
  /// [OrblitFog.density], per metre.
  final double? fogDensity;

  /// [OrblitFog.colour], linear RGB.
  final Vector3? fogColour;

  /// [OrblitFog.height] and [OrblitFog.heightFalloff].
  final double? fogHeight;
  final double? fogHeightFalloff;

  /// [OrblitFog.distance]: how far in front of the camera the fog begins.
  final double? fogStart;

  /// [OrblitFog.maximumOpacity].
  ...
  /// [OrblitSky.ambient]: how much light the sky casts, in lux.
  ///
  ...
  /// Ignored by the renderer while an [OrblitEnvironment] is lighting the
  /// scene, as the sky's ambient always is — use [environmentIntensity] then.
  final double? ambient;

  /// [OrblitSky.colour]: the colour the ambient light is.
  final Vector3? skyColour;

  /// [OrblitEnvironment.intensity], in lux.
  final double? environmentIntensity;

  /// [OrblitEnvironment.rotation], in radians about the vertical.
  final double? environmentRotation;

  /// [OrblitBloom.strength]. Turns bloom on if the scene had it off, rising
  /// from nothing, so a volume can make its lamps glow without the courtyard
  /// outside starting to.
  final double? bloomStrength;

  /// The grade: [OrblitGrading.saturation], [OrblitGrading.contrast], the two
  /// white balance axes [OrblitGrading.temperature] and [OrblitGrading.tint],
  /// and [OrblitGrading.midtones] as a colour the middle of the image is
  /// multiplied by.
  ///
...
class OrblitEnvironmentVolume {
  ...
  OrblitEnvironmentVolume.sphere({
    ...
    this.weight = 1,
    required this.overrides,
  }) : shape = OrblitVolumeShape.sphere,
  ...
  /// What it changes.
  final OrblitEnvironmentOverrides overrides;
...
class OrblitEnvironmentSettings {
  ...
  factory OrblitEnvironmentSettings.of(OrblitScene scene) {
    final fog = scene.fog;
    final bloom = scene.post.bloom;
    final grading = scene.post.grading;
    return OrblitEnvironmentSettings(
  ...
  ///
  /// Lowest [OrblitEnvironmentVolume.priority] first and highest last, ties
  ...
  OrblitEnvironmentSettings resolve(
