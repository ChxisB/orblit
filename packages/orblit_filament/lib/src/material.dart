...
/// [OrblitMaterial]. What genuinely differs is whether light is consulted at
/// all.
enum OrblitShading {
  ...
  /// graded. Set [OrblitMaterial.video] to the key of the video to show;
  /// [OrblitMaterial.baseColour] tints it, so a screen can be dimmed or faded
  ...
  ///
  /// [OrblitMaterial.baseColour] is the colour a shadow lands in and how dark
  ...
  /// definition, and [OrblitMaterial.blend] is ignored. Give the object
...
enum OrblitBlend {
  ...
  /// alpha against [OrblitMaterial.maskThreshold]. Leaves, chain-link, a
...
enum OrblitCulling {
  ...
  /// [OrblitMaterial.doubleSided] exists: culling nothing shows the back face
...
enum OrblitWrap {
  /// Tiles. What [OrblitMaterial.tiling] above one is for.
  repeat,
...
/// KTX2 are understood.
class OrblitTexture {
  const OrblitTexture(this.path, {this.srgb = true});
  ...
  factory OrblitTexture.ofTarget(String target) =>
      OrblitTexture('$targetScheme$target', srgb: false);

  /// What a target's path starts with.
  static const String targetScheme = 'orblit:target/';

  /// Absolute path to the image, or `orblit:target/<name>` for what a pass
  /// drew.
  final String path;
  ...
  bool operator ==(Object other) =>
      other is OrblitTexture && other.path == path && other.srgb == srgb;
...
class OrblitWind {
  const OrblitWind({this.bearing = 0, this.speed = 0, this.strength = 1});

  /// Still air. The vertex stage returns before doing any work.
  static const OrblitWind none = OrblitWind();
  ...
  OrblitWind copyWith({double? bearing, double? speed, double? strength}) =>
      OrblitWind(
...
enum OrblitBlendMode {
  ...
  const OrblitBlendMode(this.label);

  final String label;
}

class OrblitMaterial {
  const OrblitMaterial({
    required this.key,
    this.shading = OrblitShading.lit,
    this.blend = OrblitBlend.opaque,
    this.culling = OrblitCulling.back,
    ...
    this.depthBias = 0.0,
    this.wrap = OrblitWrap.repeat,
    this.filter = OrblitFilter.smooth,
    this.video,
    this.screenMapped = false,
    this.blendMode = OrblitBlendMode.none,
  ...
  final OrblitShading shading;
  final OrblitBlend blend;
  final OrblitCulling culling;
  ...
  /// How this surface answers the wind. [OrblitWind.none] leaves it rigid,
  /// which is what almost everything is.
  final OrblitWind wind;
  ...
  /// The alpha below which a pixel is thrown away, for [OrblitBlend.masked].
  ...
  final OrblitWrap wrap;
  final OrblitFilter filter;

  /// The key of the video this surface shows, for [OrblitShading.video].
  ...
  ///
  /// Only [OrblitShading.unlit] honours it. A mirror carries its lighting in
  ...
  final OrblitBlendMode blendMode;

  /// How much of the second surface, from none to all of it.
  final double blendAmount;

  /// How abruptly [OrblitBlendMode.maskedDepth] hands over.
  ///
  ...
  ///
  /// The mask is read from red, and in [OrblitBlendMode.maskedDepth] it is a
  /// height rather than an opacity.
  final OrblitTexture? blendBaseColourMap;
  final OrblitTexture? blendMaskMap;
  ...
  OrblitMaterial copyWith({
    OrblitShading? shading,
    OrblitBlend? blend,
    OrblitCulling? culling,
    bool? doubleSided,
    Vector4? baseColour,
    double? metallic,
    double? roughness,
