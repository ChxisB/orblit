...
- **A display's own half of the quality question.** `OrblitDisplay` states a
  panel's size and shape — `OrblitDisplay.handheld` is a Steam Deck's 1280 by
  800, which is 16:10 rather than the 16:9 most code assumes — and
  `OrblitPipeline.forDisplay` moves the three dials that depend on how many
  ...
  and adds no rung between the four `OrblitDetail` already has.
## 0.24.0
...
- **The renderer draws on Windows.** A Win32 plugin under `windows/` puts the
  ...
  Flutter's Windows embedder, speaking the same `orblit_filament` channel with
  the same method names and wire shapes — so `OrblitView` and every other Dart
  caller work there unchanged, and no shared C++ was touched to get it.
  `OrblitView` now lists Windows among the platforms it draws on rather than
  ...
  and `windows/orblit_viewport.h` sets out exactly what was and was not checked,
...
- **The renderer draws on Linux.** A GTK plugin under `linux/` puts the same
  ...
  embedder, speaking the same `orblit_filament` channel with the same method
  names and wire shapes — so `OrblitView` and every other Dart caller work
  there unchanged, and no shared C++ was touched to get it. `OrblitView` now
  ...
  inside it for the length of one callback. `linux/orblit_viewport.h` records
...
- Native backend measurement: the headless host accepts `--benchmark` with
  ...
  C function `orblit_renderer_rendered_frames` excludes skipped/detached
  draws without changing `orblit_stats` layout. The fork WebGPU build can
...
- **Instance batching is on by default.** `OrblitScene.batching` now defaults to
  true, as does the renderer a host drives through the C ABI without ever
  calling `orblit_renderer_set_batching`, and the Swift and Kotlin plugins when
...
- **Disposing a renderer with a batch group alive no longer aborts.**
  ...
  what `OrblitViewport.dispose` does on every scene widget that goes away — and
...
- **A depth prepass, off by default.** With `OrblitScene.depthPrepass` on, every
  ...
  **It ships off, including off Apple.** Defaulting it on wherever the backend
  ...
  by `OrblitScene.batching` gets no prepass either, so the two switches do not
  compound.
- **A Gaussian splat capture's colour now changes with the view.** The
  ...
  `OrblitSplats.harmonics` says how many bands to read: 0, 1, 2 or 3, and 2 by
...
- **Direct light reaches the web build again.** The renderer core in a browser
  ...
  `setup.sh` takes `ORBLIT_MATC` and records which matc built the headers in
...
- **Environment volumes.** `OrblitScene.volumes` takes `OrblitEnvironmentVolume`s
  — a turned box or a sphere, with a blend distance, a priority and a weight —
  whose `OrblitEnvironmentOverrides` change the fog, exposure, sky ambient and
  ...
  pure Dart resolver (`OrblitScene.resolved()`) when the scene is sent, so the
...
- **Projected decals.** `OrblitScene.decals` takes up to 32 `OrblitDecal`s, each
...
- **Gaussian splats.** `OrblitSplats` draws a 3D Gaussian splat capture from the
  reference trainer's binary `.ply` or a compact `.splat`, or a cloud made in
  Dart with `OrblitSplats.pack`. Each splat is projected with the EWA
...
- **Selection outlines.** `OrblitScene.outline` takes an `OrblitOutline`: a set of
  object keys, an active one drawn brighter, colours, a width in pixels, and how
  to draw the parts other objects hide (`OrblitOccluded.shown`, `faint`,
...
- **Instance batching, off by default.** With `OrblitScene.batching` on,
  objects sharing a mesh, a material and their shadow and layer flags are
  drawn as instanced renderables Orblit builds itself — up to sixty-four
...
- **God rays and screen distortion.** `OrblitScene.godRays` adds shafts of light
  ...
  `OrblitScene.distortions` takes shockwaves, heat haze and a lens warp, summed
  ...
  render-graph effects, `OrblitEffect.godRays` and `OrblitEffect.distortion`; a
  scene with no graph of its own has the passes put in for it, and neither
  costs anything while off.

- **Motion blur, from a velocity buffer.** `OrblitMotionBlur().graph()` draws
...
- **The black frames that made batching experimental are a fault in Filament,
  ...
  test sits on Orblit's Filament fork, and against a Filament built with it
...
- **Android.** The plugin has an Android implementation: Kotlin and JNI over
  the same C ABI and the same `orblit_filament` channel protocol as the Swift
  ...
  says so. Two calls join the C ABI, `orblit_renderer_attach_surface` and
...
- `OrblitScene.copyWith` keeps `probes` and `field`, which it used to drop
  silently.

- **The renderer is portable C++.** Everything it does is now `orblit::Renderer`
  (`OrblitRendererCore.h`/`.cpp`), with no Objective-C and no Apple header in
  it; the Objective-C `OrblitRenderer` is a thin wrapper, so the Swift plugin is
  ...
  Android, Linux and Windows, OpenGL on the web — and `ORBLIT_BACKEND`
  overrides it; a backend with no driver is skipped rather than crashing.
  `include/orblit_renderer.h` is a C ABI for hosts with neither Objective-C nor
  ...
  `ORBLIT_MATC_BACKENDS` compiles the materials for other backends; every
...
- **Clear coat, anisotropy and sheen.** `OrblitMaterial.clearCoat`,
...
- **Wind, as a vertex stage.** `OrblitWind` on a material says how much that
...
- **Reflection probes.** `OrblitProbe` is the scene photographing itself from a
...
- **Rectangular area lights.** `OrblitLightKind.area` is a panel that emits
...
- **`OrblitField`: light kept in the world rather than on the screen.** A
...
- **`OrblitEffect.copy`**, the plainest pass there is and the one that makes the
...
- **`OrblitEffect.bounce`: one bounce of light, taken from the picture already
...
- **`OrblitScene.copyWith`.** A scene is stated whole every frame, which made
...
- **SMAA works.** `OrblitEffect.smaaWeights` and `OrblitEffect.smaaBlend` finish
...
- `OrblitEffect.smaaEdges`, the first of SMAA's three passes: it writes a
...
- `OrblitPassKind.effect` runs a material over every pixel of what another pass
  ...
  effect a pass runs is `OrblitPass.effect`, one of a set the renderer knows,
  because an effect needs a compiled shader and compiling one at runtime is a
  much larger door than this.

- `OrblitEffect.sharpen`, the first of them: a contrast-adaptive sharpen that
...
- `setup.sh` can build against a Filament we own. `ORBLIT_FILAMENT_SRC` points
  ...
  in Orblit's own render graph against the same public API, which is how the
...
- `OrblitObject.morphWeights` dials in a mesh's shapes. A morph target is a
...
- **Draco and meshopt glTF import already worked**, and now there is a
  reference saying so. Both decoders are linked into the archive Orblit ships
...
- `OrblitFade.none` joins `sink` and `shrink`: the member does not change shape
...
- A population says how its members go when they pass its `range`.
  `OrblitFade.sink` is what happened before and stays the default;
  `OrblitFade.shrink` draws a member in towards its own centre instead.
...
- A material can carry a second surface and choose between them per pixel,
  ...
  on `OrblitMaterial.blendMode` — `linear` fades everywhere, `masked` follows a
...
- Presentation sits behind an `OrblitSurface` interface rather than being
...
- First cut of `orblit_filament`. Pre-alpha: everything is subject to change.
