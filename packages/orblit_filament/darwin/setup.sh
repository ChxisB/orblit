...
# or a reflection probe is written in Orblit's own render graph against the same
...
if [ -n "${ORBLIT_FILAMENT_SRC:-}" ]; then
  BUILT="$ORBLIT_FILAMENT_SRC/out/release/filament"
  if [ ! -d "$BUILT/lib" ]; then
    echo "orblit_filament: ORBLIT_FILAMENT_SRC is set but $BUILT/lib is missing."
    echo "  Build it:  cd $ORBLIT_FILAMENT_SRC && ./build.sh -p desktop -i release"
    exit 1
  fi
  echo "orblit_filament: using the Filament built at $BUILT"
  mkdir -p "$SDK_DIR"
...
# headers an Apple or Linux release build includes. ORBLIT_GENERATED_SET is a
# name, not a path, so an accidental absolute path cannot escape this tree.
GENERATED_ROOT="orblit_filament/Sources/orblit_filament_native/generated"
if [ -n "${ORBLIT_GENERATED_SET:-}" ]; then
  MATERIAL_SET="$ORBLIT_GENERATED_SET"
...
case "$MATERIAL_SET" in
  ''|.|..|*/*)
    echo "orblit_filament: ORBLIT_GENERATED_SET must be a simple directory name" >&2
    exit 1
    ;;
esac
GENERATED="$GENERATED_ROOT/$MATERIAL_SET"
mkdir -p "$GENERATED"
echo "orblit_filament: generated materials -> $GENERATED"
...
for tex in AreaTex SearchTex; do
  if [ ! -s "$GENERATED/$tex.h" ]; then
    echo "orblit_filament: fetching SMAA $tex"
    mkdir -p "$GENERATED"
...
if [ ! -s "$LTC_HEADER" ]; then
  echo "orblit_filament: fetching the LTC tables"
  curl -fsSL -o /tmp/orblit_ltc.js "$LTC_FROM"
...
#
# Which backends the materials carry shaders for: ORBLIT_MATC_BACKENDS, a list
...
# to prevent.
ORBLIT_MATC_BACKENDS="${ORBLIT_MATC_BACKENDS:-metal}"
MATC_API=""
for api in ${ORBLIT_MATC_BACKENDS//,/ }; do
  case "$api" in
    metal|vulkan|opengl|webgpu|all) MATC_API="$MATC_API -a $api" ;;
    *)
      echo "orblit_filament: ORBLIT_MATC_BACKENDS names '$api', which is not"
...
# Which matc compiles them: ORBLIT_MATC, a path to one, defaulting to the matc
...
# constant (upstream #10390, in the fork since it merged orblit-main). So a
...
MATC="${ORBLIT_MATC:-$FILAMENT/bin/matc}"
if [ ! -x "$MATC" ]; then
  echo "orblit_filament: no matc at $MATC" >&2
  exit 1
fi
...
compile() {
  ...
  echo "orblit_filament: compiling $name"
  local input="$source"
  if [ -n "$blend" ]; then
    input="/tmp/orblit_src_$name.mat"
...
XCFRAMEWORK="orblit_filament/third_party/Filament.xcframework"
STAMP="orblit_filament/third_party/.filament-version"
...
if [ "$(cat "$STAMP" 2>/dev/null || true)" != "$WANT" ]; then
  echo "orblit_filament: packaging Filament as a framework"
  ...
  ios_slice() {
    ...
    for name in "${LIBS[@]}"; do
      ...
      if [ ! -f "$path" ]; then
        echo "orblit_filament: no $identifier slice for $name"
        exit 1
      fi
      archives+=("$path")
    done
...
echo "orblit_filament: setup complete"
