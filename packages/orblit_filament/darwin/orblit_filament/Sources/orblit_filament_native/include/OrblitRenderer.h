...
@interface OrblitRenderer : NSObject
...
/// which this deliberately does not use; see OrblitScene.batching on the Dart
...
/// written out in OrblitDecals.h. `images` is `count` indices into `paths`,
...
/// settings and every distortion end to end, as OrblitGodRays and
/// OrblitDistortion pack them. Kept until the next scene; a row that is not
