...
class AppleSurface final : public OrblitSurface {
 public:
  ...
  void writeFrame(int index) override {
    ...
    NSString* where =
        [NSTemporaryDirectory() stringByAppendingPathComponent:@"orblit_frame.png"];
    ...
    NSLog(@"[orblit] frame (%zux%zu) -> %s : %@", CVPixelBufferGetWidth(buffer),
...
OrblitSurface* OrblitCreateSurface() { return new AppleSurface(); }
