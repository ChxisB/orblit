...
class OrblitSurface {
 public:
  ...
  /// says where. Diagnostics, for ORBLIT_DUMP_FRAME.
  ///
...
/// one so far (OrblitSurfaceApple.mm).
OrblitSurface* OrblitCreateSurface();

/// Frames drawn into an offscreen swap chain nobody sees, and read back with
/// orblit::Renderer::requestCapture. Tests, servers, and a console host that
/// has not opened a window. Every platform (OrblitSurfaceHeadless.cpp).
