...
// and answered twice — OrblitPlatformApple.mm with the frameworks the Apple
// build has always used, and OrblitPlatform.cpp with the standard library and
// what Filament's own release ships — so the renderer asks without knowing
// which platform answered.
//
// Not in include/, for the reason OrblitSurface.h is not: that directory is
...
// ORBLIT_PLATFORM_PORTABLE is defined — which is how the portable answers are
// compiled and run on a Mac, to prove they work somewhere they can be looked
// at before they are needed somewhere they cannot.
#if defined(__APPLE__) && !defined(ORBLIT_PLATFORM_PORTABLE)
#define ORBLIT_PLATFORM_APPLE 1
#else
#define ORBLIT_PLATFORM_APPLE 0
#endif

#if defined(__GNUC__) || defined(__clang__)
#define ORBLIT_PRINTF(which, first) \
