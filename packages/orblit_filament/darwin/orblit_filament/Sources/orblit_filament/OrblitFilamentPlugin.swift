...
// file serves both.
#if canImport(orblit_filament_native)
...
private final class Viewport {
  let textureId: Int64
  private let renderer: OrblitRenderer
  ...
  init(textureId: Int64, renderer: OrblitRenderer,
...
private struct Scene {
  ...
  /// OrblitRenderGraph on the Dart side and the constants in the renderer.
  fileprivate static let environmentStride = 4
  fileprivate static let passStride = 13
  fileprivate static let targetStride = 6
  /// God rays and one distortion. Must match OrblitGodRays.stride and
  /// OrblitDistortion.stride, and kGodRayStride and kDistortionStride in
  /// ScreenEffects.h.
  ...
  /// Must match OrblitDecal.stride and kDecalStride in OrblitDecals.h.
  private static let decalStride = 22
  /// Must match OrblitOutline.stride in Dart and kOutlineParams in the
  /// renderer's outline.
...
public class OrblitFilamentPlugin: NSObject, FlutterPlugin {
  private let registry: FlutterTextureRegistry
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "create":
      ...
      let made = engineThread.sync {
        OrblitRenderer(width: UInt32(width), height: UInt32(height))
      }
      ...
      let texture = OrblitTexture(renderer: renderer)
