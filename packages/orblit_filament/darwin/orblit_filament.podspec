...
without a trip through the CPU.
  ...
  generated_set = ENV['ORBLIT_GENERATED_SET']
  if generated_set.nil?
    generated_set = ENV['ORBLIT_FILAMENT_SRC'] ? 'darwin-source' : 'darwin-release'
  end
  raise "ORBLIT_GENERATED_SET must be a simple directory name" unless generated_set.match?(/\A[A-Za-z0-9_-]+\z/)
  ...
  s.vendored_frameworks = 'orblit_filament/third_party/Filament.xcframework'
  ...
  s.pod_target_xcconfig = {
    ...
    'HEADER_SEARCH_PATHS' => [
      ...
      '"$(PODS_TARGET_SRCROOT)/orblit_filament/Sources/orblit_filament_native/include"',
      ...
      '"$(PODS_TARGET_SRCROOT)/orblit_filament/Sources/orblit_filament_native/generated/' + generated_set + '"',
    ].join(' '),
  }
